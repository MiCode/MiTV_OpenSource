...
{
	struct hd_struct *p = dev_to_part(dev);
	int MAJOR = 0; //Main equipment number
	int MINOR = 0; //Secondary equipment number
	MAJOR = MAJOR(part_devt(p));
	MINOR = MINOR(part_devt(p));
	if((EMMC_STATIC_32G == 1) && (MAJOR == MMC_BLOCK_MAJOR) && (MINOR == 0))
	{
		printk("LDY_MAJOR_MINOR_STATIC_32G:%d %d %d\n",MAJOR, MINOR, EMMC_STATIC_32G);
		return sprintf(buf, "%llu\n",(unsigned long long)part_nr_sects_read(p)/2);
	}else{
