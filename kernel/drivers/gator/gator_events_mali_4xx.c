...
GATOR_DEFINE_PROBE(mali_hw_counter,
...
GATOR_DEFINE_PROBE(mali_sw_counters,
		   TP_PROTO(pid_t pid, pid_t tid, void *surface_id,
			    unsigned int *counters))
{
	...
	for (i = FIRST_SW_COUNTER; i <= LAST_SW_COUNTER; i++) {
		if (counter_enabled[i])
			atomic_add(counters[i - FIRST_SW_COUNTER],
				   &counter_data[i]);
	}
}
 ...
 */
static int create_fs_entry(struct super_block *sb, struct dentry *root,
			   const char *name, int event, int create_event_item)
{
	struct dentry *dir;
...
static void initialise_version_info(void)
{
	...
	if (mali_profiling_get_mali_version_symbol) {
		...
		pr_err
		    ("gator: mali online _mali_profiling_get_mali_version symbol not found\n");
		pr_err
		    ("gator:  check your Mali DDK version versus the GATOR_MALI_INTERFACE_STYLE setting\n");
	}
}
#endif
...
static int create_files(struct super_block *sb, struct dentry *root)
{
	...
	for (event = 0; event < ARRAY_SIZE(mali_activity); event++) {
		if (gator_mali_create_file_system
		    (mali_name, mali_activity_names[event], sb, root,
		     &mali_activity[event], NULL) != 0)
			return -1;
	}
	...
	for (core_id = 0; core_id < n_vp_cores; core_id++) {
		...
		snprintf(buf, sizeof(buf), "ARM_Mali-%s_VP_%d_active",
			 mali_name, core_id);
		...
		for (counter_number = 0; counter_number < 2; counter_number++) {
			int counter_id =
			    COUNTER_VP_0_C0 + (2 * core_id) + counter_number;

			snprintf(buf, sizeof(buf), "ARM_Mali-%s_VP_%d_cnt%d",
				 mali_name, core_id, counter_number);
	...
	for (core_id = 0; core_id < n_fp_cores; core_id++) {
		...
		snprintf(buf, sizeof(buf), "ARM_Mali-%s_FP_%d_active",
			 mali_name, core_id);
		if (create_fs_entry(sb, root, buf, activity_counter_id, 0) != 0)
			return -1;
		...
		for (counter_number = 0; counter_number < 2; counter_number++) {
			int counter_id =
			    COUNTER_FP_0_C0 + (2 * core_id) + counter_number;

			snprintf(buf, sizeof(buf), "ARM_Mali-%s_FP_%d_cnt%d",
				 mali_name, core_id, counter_number);
			if (create_fs_entry(sb, root, buf, counter_id, 1) != 0)
				return -1;
		}
	...
	for (core_id = 0; core_id < n_l2_cores; core_id++) {
		for (counter_number = 0; counter_number < 2; counter_number++) {
			int counter_id =
			    COUNTER_L2_0_C0 + (2 * core_id) + counter_number;

			snprintf(buf, sizeof(buf), "ARM_Mali-%s_L2_%d_cnt%d",
				 mali_name, core_id, counter_number);
			if (create_fs_entry(sb, root, buf, counter_id, 1) != 0)
				return -1;
		}
	...
	for (event = FIRST_SW_COUNTER; event <= LAST_SW_COUNTER; event++) {
		snprintf(buf, sizeof(buf), "ARM_Mali-%s_SW_%d", mali_name,
			 event - FIRST_SW_COUNTER);
 ...
 */
static void (*mali_get_counters) (unsigned int *, unsigned int *,
				  unsigned int *, unsigned int *);
static u32(*mali_get_l2_counters) (struct _mali_profiling_l2_counter_values *
				   values);
 ...
 */
static int is_any_counter_enabled(unsigned int first_counter,
...
static void init_counters(unsigned int from_counter, unsigned int to_counter)
{
	...
	if (mali_set_hw_event) {
		pr_debug
		    ("gator: mali online _mali_profiling_set_event symbol @ %p\n",
		     mali_set_hw_event);

		for (counter_id = from_counter; counter_id <= to_counter;
		     counter_id++) {
			if (counter_enabled[counter_id])
				mali_set_hw_event(counter_id,
						  counter_event[counter_id]);
			else
		...
		pr_err
		    ("gator: mali online _mali_profiling_set_event symbol not found\n");
	}
}
...
static void mali_counter_initialize(void)
{
	...
	if (mali_control) {
		...
		unsigned int resize_factor =
		    (counter_event[COUNTER_FILMSTRIP] >> 8) & 0xff;

		pr_debug
		    ("gator: mali online _mali_profiling_control symbol @ %p\n",
		     mali_control);

		mali_control(SW_COUNTER_ENABLE,
			     (is_any_counter_enabled
			      (FIRST_SW_COUNTER, LAST_SW_COUNTER) ? 1 : 0));
		mali_control(FBDUMP_CONTROL_ENABLE,
			     (counter_enabled[COUNTER_FILMSTRIP] ? 1 : 0));
		...
		pr_err
		    ("gator: mali online _mali_profiling_control symbol not found\n");
	}

	mali_get_counters = symbol_get(_mali_profiling_get_counters);
	if (mali_get_counters)
		pr_debug
		    ("gator: mali online _mali_profiling_get_counters symbol @ %p\n",
		     mali_get_counters);
	else
		pr_debug
		    ("gator: mali _mali_profiling_get_counters symbol not defined\n");

	mali_get_l2_counters = symbol_get(_mali_profiling_get_l2_counters);
	if (mali_get_l2_counters)
		pr_debug
		    ("gator: mali online _mali_profiling_get_l2_counters symbol @ %p\n",
		     mali_get_l2_counters);
	else
		pr_debug
		    ("gator: mali _mali_profiling_get_l2_counters symbol not defined\n");
...
static void mali_counter_deinitialize(void)
{
	...
	if (mali_set_hw_event) {
		...
		pr_debug
		    ("gator: mali offline _mali_profiling_set_event symbol @ %p\n",
		     mali_set_hw_event);
		...
		pr_err
		    ("gator: mali offline _mali_profiling_set_event symbol not found\n");
	}
	...
	if (mali_control) {
		pr_debug
		    ("gator: mali offline _mali_profiling_control symbol @ %p\n",
		     mali_control);
		...
		pr_err
		    ("gator: mali offline _mali_profiling_control symbol not found\n");
	}
...
static int start(void)
{
	...
	if (GATOR_REGISTER_TRACE(mali_hw_counter)) {
		pr_err
		    ("gator: mali_hw_counter tracepoint failed to activate\n");
		return -1;
	}
	...
	if (GATOR_REGISTER_TRACE(mali_sw_counters)) {
		pr_err
		    ("gator: mali_sw_counters tracepoint failed to activate\n");
		return -1;
	}
...
static void dump_counters(unsigned int from_counter, unsigned int to_counter,
			  unsigned int *len)
{
	unsigned int counter_id;
...
static int read(int **buffer, bool sched_switch)
{
	...
	if (n_l2_cores > 0
	    && is_any_counter_enabled(COUNTER_L2_0_C0,
				      COUNTER_L2_0_C0 + (2 * n_l2_cores))) {
		...
		if (mali_get_l2_counters) {
			unavailable_l2_caches =
			    mali_get_l2_counters(&cache_values);
		} else if (mali_get_counters) {
			per_core = &cache_values.cores[0];
			mali_get_counters(&per_core->source0, &per_core->value0,
					  &per_core->source1,
					  &per_core->value1);
		} else {
		...
		for (cache_id = 0; cache_id < n_l2_cores; cache_id++) {
			unsigned int counter_id_0 =
			    COUNTER_L2_0_C0 + (2 * cache_id);
	...
	dump_counters(COUNTER_VP_0_C0, COUNTER_VP_0_C0 + (2 * n_vp_cores) - 1,
		      &len);
	dump_counters(COUNTER_FP_0_C0, COUNTER_FP_0_C0 + (2 * n_fp_cores) - 1,
		      &len);
...
extern void gator_events_mali_log_dvfs_event(unsigned int frequency_mhz,
...
int gator_events_mali_init(void)
{
	...
	gator_mali_initialise_counters(mali_activity,
				       ARRAY_SIZE(mali_activity));
