...
static int gator_events_sched_create_files(struct super_block *sb,
					   struct dentry *root)
{
	struct dentry *dir;
	int i;
	...
	for (i = 0; i < gator_cluster_count; i++) {
		snprintf(buf, sizeof(buf), "%s_switch",
			 gator_clusters[i]->pmnc_name);
		dir = gatorfs_mkdir(sb, root, buf);
		if (!dir)
			return -1;
		gatorfs_create_ulong(sb, dir, "enabled",
				     &sched_switch_enabled[i]);
...
static int gator_events_sched_start(void)
{
 ...
 sched_switch_exit:
	pr_err
	    ("gator: scheduler event tracepoints failed to activate, please verify that tracepoints are enabled in the linux kernel\n");
