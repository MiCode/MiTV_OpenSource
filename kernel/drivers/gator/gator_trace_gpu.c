...
static int mali_activity_index(int core, int key)
{
	...
	for (i = 0; i < ARRAY_SIZE(mali_activities); ++i) {
		if ((mali_activities[i].core == core)
		    && (mali_activities[i].key == key))
			break;
		if ((mali_activities[i].core == 0)
		    && (mali_activities[i].key == 0)) {
...
static void mali_activity_enqueue(int core, int key, int activity, int pid)
{
	...
	if (count > 1)
		 ...
		 */
		gator_marshal_activity_switch(core, key,
					      mali_activities[i].last_activity,
					      mali_activities[i].last_pid);
	else
...
static void mali_activity_stop(int core, int key)
{
	...
	if (count)
		gator_marshal_activity_switch(core, key, last_activity,
					      last_pid);
}

static void mali_activity_clear(struct mali_counter mali_activity[],
				size_t mali_activity_size)
{
	int activity;
	int cores;
	int core;
	...
	for (activity = 0; activity < mali_activity_size; ++activity) {
		...
		for (core = 0; core < cores; ++core) {
			if (mali_activity[activity].enabled) {
				preempt_disable();
				gator_marshal_activity_switch(core,
							      mali_activity
							      [activity].key, 0,
							      0);
				preempt_enable();
...
GATOR_DEFINE_PROBE(mali_timeline_event,
		   TP_PROTO(unsigned int event_id, unsigned int d0,
	...
	case EVENT_TYPE_START:
		if (component == EVENT_CHANNEL_VP0) {
			...
			if (mali_activity[1].enabled)
				mali_activity_enqueue(0, mali_activity[1].key,
						      1, d1);
		} else if (component >= EVENT_CHANNEL_FP0
			   && component <= EVENT_CHANNEL_FP7) {
			/* tgid = d0; pid = d1; */
			if (mali_activity[0].enabled)
				mali_activity_enqueue(component -
						      EVENT_CHANNEL_FP0,
						      mali_activity[0].key, 1,
						      d1);
		}
		break;
...
GATOR_DEFINE_PROBE(mali_job_slots_event,
		   TP_PROTO(unsigned int event_id, unsigned int tgid,
			    unsigned int pid, unsigned char job_id))
#else
GATOR_DEFINE_PROBE(mali_job_slots_event,
		   TP_PROTO(unsigned int event_id, unsigned int tgid,
			    unsigned int pid))
	...
	if (unit != GPU_UNIT_NONE) {
		...
		case EVENT_TYPE_START:
			if (mali_activity[component].enabled)
				mali_activity_enqueue(0,
						      mali_activity[component].
						      key, 1,
						      (pid != 0 ? pid : tgid));
			break;
		case EVENT_TYPE_STOP:
