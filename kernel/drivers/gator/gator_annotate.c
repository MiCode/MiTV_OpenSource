...
static int annotate_copy(struct file *file, char const __user * buf,
			 size_t count)
{
	int cpu = 0;
	...
	if (file == NULL) {
		/* copy from kernel */
		memcpy(&per_cpu(gator_buffer, cpu)[ANNOTATE_BUF][write], buf,
		       count);
	} else {
		/* copy from user space */
		if (copy_from_user
		    (&per_cpu(gator_buffer, cpu)[ANNOTATE_BUF][write], buf,
		     count) != 0)
			return -1;
	}
	per_cpu(gator_buffer_write, cpu)[ANNOTATE_BUF] =
	    (write + count) & gator_buffer_mask[ANNOTATE_BUF];

	return 0;
}

static ssize_t annotate_write(struct file *file, char const __user * buf,
			      size_t count_orig, loff_t * offset)
{
	int pid, cpu, header_size, available, contiguous, length1, length2,
	    size, count = count_orig & 0x7fffffff;
	...
	if (interrupt_context) {
		pr_warning
		    ("gator: Annotations are not supported in interrupt context. Edit gator_annotate.c in the gator driver to enable annotations in interrupt context.\n");
		return -EINVAL;
	}
 ...
 retry:
	...
	if (size <= 0) {
		...
		wait_event_interruptible(gator_annotate_wait,
					 buffer_bytes_available(cpu,
								ANNOTATE_BUF) >
					 header_size || !collect_annotations);
	...
	if (per_cpu(gator_buffer, cpu)[ANNOTATE_BUF]) {
		...
		gator_buffer_write_packed_int(cpu, ANNOTATE_BUF,
					      get_physical_cpu());
		...
		if (length2 > 0
		    && annotate_copy(file, &buf[length1], length2) != 0) {
			size = -EINVAL;
			goto annotate_write_out;
		}
...
static int annotate_release(struct inode *inode, struct file *file)
{
	...
	if (per_cpu(gator_buffer, cpu)[ANNOTATE_BUF]
	    && buffer_check_space(cpu, ANNOTATE_BUF,
				  MAXSIZE_PACK64 + 3 * MAXSIZE_PACK32)) {
...
static int gator_annotate_create_files(struct super_block *sb,
				       struct dentry *root)
{
	return gatorfs_create_file_perm(sb, root, "annotate", &annotate_fops,
					0666);
}
