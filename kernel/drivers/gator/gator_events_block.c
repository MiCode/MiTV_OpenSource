...
GATOR_DEFINE_PROBE(block_rq_complete,
...
static int gator_events_block_create_files(struct super_block *sb,
					   struct dentry *root)
{
	struct dentry *dir;
...
static int gator_events_block_start(void)
{
 ...
 fail_block_rq_exit:
	pr_err
	    ("gator: block event tracepoints failed to activate, please verify that tracepoints are enabled in the linux kernel\n");
...
static int gator_events_block_read(int **buffer, bool sched_switch)
{
	...
	len = 0;
	if (block_rq_wr_enabled
	    && (value = atomic_read(&blockCnt[BLOCK_RQ_WR])) > 0) {
	...
	if (block_rq_rd_enabled
	    && (value = atomic_read(&blockCnt[BLOCK_RQ_RD])) > 0) {
