...
static ulong
    sched_activity_enabled[ARRAY_SIZE(sched_activity_event_names)]
    [GATOR_CLUSTER_COUNT];
static ulong
    sched_activity_keys[ARRAY_SIZE(sched_activity_event_names)]
    [GATOR_CLUSTER_COUNT];
	...
	for (i = 0; i < ARRAY_SIZE(sched_activity_event_names); ++i) {
		for (j = 0; j < gator_cluster_count; j++) {
			snprintf(buf, sizeof(buf), "%s_%s",
				 gator_clusters[j]->pmnc_name,
				 sched_activity_event_names[i]);
			dir = gatorfs_mkdir(sb, root, buf);
			if (!dir)
				return -1;
			gatorfs_create_ulong(sb, dir, "enabled",
					     &sched_activity_enabled[i][j]);
			gatorfs_create_ro_ulong(sb, dir, "key",
						&sched_activity_keys[i][j]);
		}
	}
...
static void emit_pid_name(const char *comm, struct task_struct *task)
{
	...
	uint64_t *keys =
	    &(per_cpu(taskname_keys, cpu)
	      [(task->pid & 0xFF) * TASK_MAX_COLLISIONS]);
	uint64_t value;
			     ...
			     bool sched_switch)
{
	...
	if (marshal_event_header(time)) {
		...
		if (gator_live_rate > 0
		    && time >= per_cpu(gator_buffer_commit_time, cpu)) {
			static const int buftypes[] =
			    { NAME_BUF, COUNTER_BUF, BLOCK_COUNTER_BUF,
	  SCHED_TRACE_BUF, ACTIVITY_BUF };
			int i;
...
GATOR_DEFINE_PROBE(sched_process_fork,
...
GATOR_DEFINE_PROBE(sched_process_exec,
...
#else
GATOR_DEFINE_PROBE(task_rename,
...
GATOR_DEFINE_PROBE(sched_switch,
...
static int register_scheduler_tracepoints(void)
{
 ...
 fail_sched_process_fork:
	pr_err
	    ("gator: tracepoints failed to activate, please verify that tracepoints are enabled in the linux kernel\n");
...
static int gator_trace_sched_start(void)
{
	...
	for_each_present_cpu(cpu) {
		size =
		    TASK_MAP_ENTRIES * TASK_MAX_COLLISIONS * sizeof(uint64_t);
