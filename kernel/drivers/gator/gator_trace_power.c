					  ...
					  struct dentry *root)
{
	...
	if (found_nonzero_freq) {
		...
		for (i = 0; i < gator_cluster_count; i++) {
			snprintf(buf, sizeof(buf), "%s_freq",
...
GATOR_DEFINE_PROBE(cpu_frequency,
		   TP_PROTO(unsigned int frequency, unsigned int cpu))
{
	cpu = lcpu_to_pcpu(cpu);
	marshal_event_single64(cpu, power_cpu_key[gator_clusterids[cpu]],
			       frequency * 1000L);
}
...
static void gator_trace_power_online(void)
{
	...
	if (power_cpu_enabled[gator_clusterids[pcpu]])
		marshal_event_single64(pcpu,
				       power_cpu_key[gator_clusterids[pcpu]],
				       cpufreq_quick_get(lcpu) * 1000L);
}
...
static void gator_trace_power_offline(void)
{
	...
	if (power_cpu_enabled[gator_clusterids[cpu]])
		marshal_event_single(cpu, power_cpu_key[gator_clusterids[cpu]],
				     0);
}
...
static int gator_trace_power_start(void)
{
 ...
 fail_cpu_frequency_exit:
	pr_err
	    ("gator: power event tracepoints failed to activate, please verify that tracepoints are enabled in the linux kernel\n");
