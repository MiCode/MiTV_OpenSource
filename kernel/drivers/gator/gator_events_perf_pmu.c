...
static int __create_files(struct super_block *sb, struct dentry *root,
			  struct gator_attr *const attr)
{
	struct dentry *dir;
...
static int gator_events_perf_pmu_create_files(struct super_block *sb,
					      struct dentry *root)
{
	int cnt;
...
static void ebs_overflow_handler(struct perf_event *event,
...
static void __online_dispatch(int cpu, bool migrate,
			      struct gator_attr *const attr,
			      struct gator_event *const event)
{
	const char *const cpu_core_name =
	    (gator_cpus_per_core[cpu] !=
	     NULL ? gator_cpus_per_core[cpu]->core_name : "Unknown");
	const u32 cpu_cpuid =
	    (gator_cpus_per_core[cpu] !=
	     NULL ? gator_cpus_per_core[cpu]->cpuid : gator_cpuids[cpu]);
	...
	if ((gator_cpus_per_core[cpu] == NULL)
	    || ((gator_cpus_per_core[cpu] != attr->gator_cpu)
		&& (attr->gator_cpu != NULL))) {
		pr_debug
		    ("gator: Counter %s does not apply to core %i (Core: %s 0x%x, Counter: %s 0x%lx)\n",
		     attr->name, cpu, cpu_core_name, cpu_cpuid,
		     attr->gator_cpu->core_name, attr->gator_cpu->cpuid);
		return;
	} else {
	...
	pevent =
	    perf_event_create_kernel_counter(event->pevent_attr, cpu, NULL,
					     handler, NULL);
	if (IS_ERR(pevent)) {
...
static void gator_events_perf_pmu_online_dispatch(int cpu, bool migrate)
{
	...
	for (cnt = 0; cnt < attr_count; cnt++) {
		__online_dispatch(cpu, migrate, &attrs[cnt],
				  &per_cpu(events, cpu)[cnt]);
	}

	for (cnt = 0; cnt < uc_attr_count; cnt++) {
		struct cpumask *cpumask;
		GATOR_IF_UNCORE_CPUMASK(uc_attrs[cnt].uncore_pmu, cpu, cpumask) {
			__online_dispatch(cpu, migrate, &uc_attrs[cnt],
					  &uc_events[cnt]);
		}
	}
}
...
static int __start(struct gator_attr *const attr,
		   struct gator_event *const event)
{
...
static int gator_events_perf_pmu_start(void)
{
	...
	for_each_present_cpu(cpu) {
		for (cnt = 0; cnt < attr_count; cnt++) {
			if (__start(&attrs[cnt], &per_cpu(events, cpu)[cnt]) !=
			    0)
				return -1;
		}
	}
...
static void __read(int *const len, int cpu, struct gator_attr *const attr,
		   struct gator_event *const event)
{
	uint32_t delta;
...
static void gator_events_perf_pmu_uncore_init(const struct uncore_pmu *const
					      uncore_pmu, const int type)
{
	int cnt;

	if (uncore_pmu->has_cycles_counter) {
		if (uc_attr_count < ARRAY_SIZE(uc_attrs)) {
			snprintf(uc_attrs[uc_attr_count].name,
				 sizeof(uc_attrs[uc_attr_count].name),
				 "%s_ccnt", uncore_pmu->core_name);
	...
	for (cnt = 0; cnt < uncore_pmu->pmnc_counters; ++cnt, ++uc_attr_count) {
		...
		if (uc_attr_count < ARRAY_SIZE(uc_attrs)) {
			snprintf(attr->name, sizeof(attr->name), "%s_cnt%d",
				 uncore_pmu->core_name, cnt);
			attr->type = type;
			attr->gator_cpu = NULL;
...
static void gator_events_perf_pmu_cpu_init(const struct gator_cpu *const
					   gator_cpu, const int type)
{
	int cnt;
	...
	if (gator_cluster_count < ARRAY_SIZE(gator_clusters)) {
		...
		if (attr_count < ARRAY_SIZE(attrs)) {
			snprintf(attrs[attr_count].name,
				 sizeof(attrs[attr_count].name), "%s_ccnt",
				 gator_cpu->pmnc_name);
		...
		for (cnt = 0; cnt < gator_cpu->pmnc_counters;
		     ++cnt, ++attr_count) {
			struct gator_attr *const attr = &attrs[attr_count];

			if (attr_count < ARRAY_SIZE(attrs)) {
				snprintf(attr->name, sizeof(attr->name),
					 "%s_cnt%d", gator_cpu->pmnc_name, cnt);
				attr->type = type;
...
static int gator_events_perf_pmu_reread(void)
{
	...
	for (type = PERF_TYPE_MAX; type < 0xff; ++type) {
		...
		pea.config = 0xff;
		for_each_present_cpu(cpu) {
			pe = perf_event_create_kernel_counter(&pea, cpu, NULL,
							      dummy_handler,
							      NULL);
			if (IS_ERR(pe)) {
				pr_debug
				    ("gator: failed for type %i for config 0x%llx on cpu %i",
				     type, pea.config, cpu);
			} else {
				break;
			}
		...
		if (IS_ERR(pe)) {
			pea.config = 0x11;
			for_each_present_cpu(cpu) {
				pe = perf_event_create_kernel_counter(&pea, cpu,
								      NULL,
								      dummy_handler,
								      NULL);
				if (!IS_ERR(pe)) {
		...
		if (pe->pmu != NULL) {
			pr_debug("gator: detected pmu of type %i / %s",
				 pe->pmu->type, pe->pmu->name);
		}

		if (pe->pmu != NULL && type == pe->pmu->type) {
			pr_notice("gator: perf pmu: %s\n", pe->pmu->name);
			if ((uncore_pmu =
			     gator_find_uncore_pmu(pe->pmu->name)) != NULL) {
				pr_notice
				    ("gator: Adding uncore counters for %s (%s) with type %i\n",
				     uncore_pmu->core_name,
				     uncore_pmu->pmnc_name, type);
				gator_events_perf_pmu_uncore_init(uncore_pmu,
								  type);
			} else
			    if ((gator_cpu =
				 gator_find_cpu_by_pmu_name(pe->pmu->name)) !=
				NULL) {
				pr_notice
				    ("gator: Adding cpu counters for %s (%s) with type %i\n",
				     gator_cpu->core_name, gator_cpu->pmnc_name,
				     type);
	...
	for (cpu = 0; cpu < ARRAY_SIZE(gator_cpuids); ++cpu) {
		if (gator_cpuids[cpu] != ((u32) - 1)) {
			...
			const struct gator_cpu *gator_cpu =
			    gator_find_cpu_by_cpuid(gator_cpuids[cpu]);

			if (gator_cpu == NULL) {
#if defined(__arm__) || defined(__aarch64__)
				pr_err
				    ("gator: This CPU is not recognized, using the Arm architected counters\n");
#else
				pr_err
				    ("gator: This CPU is not recognized, using perf built in hardware counters\n");
#endif
			...
			if (!found_cpu) {
				pr_notice
				    ("gator: Adding cpu counters (based on cpuid) for %s\n",
				     gator_cpu->core_name);
#if defined(__arm__) || defined(__aarch64__)
				gator_events_perf_pmu_cpu_init(gator_cpu,
							       PERF_TYPE_RAW);
#else
				gator_events_perf_pmu_cpu_init(gator_cpu,
							       PERF_TYPE_HARDWARE);
	...
	for (cpu = 0; cpu < ARRAY_SIZE(gator_cpuids); ++cpu) {
		if (gator_cpus_per_core[cpu] != NULL) {
			pr_notice("gator: Using %s (0x%lx) for cpu %i\n",
				  gator_cpus_per_core[cpu]->core_name,
				  gator_cpus_per_core[cpu]->cpuid, cpu);
		}
	}
	...
	if (attr_count > CNTMAX) {
		pr_err
		    ("gator: Too many perf counters, please increase CNTMAX\n");
		return -1;
	}

	if (uc_attr_count > UCCNT) {
		pr_err
		    ("gator: Too many perf uncore counters, please increase UCCNT\n");
		return -1;
	}
