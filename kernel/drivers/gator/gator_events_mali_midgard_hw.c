...
static uint32_t(*kbase_gator_instr_hwcnt_dump_irq_symbol) (struct
							   kbase_gator_hwcnt_handles
							   *);
static uint32_t(*kbase_gator_instr_hwcnt_dump_complete_symbol) (struct
								kbase_gator_hwcnt_handles
								*,
								uint32_t *
								const);
static struct kbase_gator_hwcnt_handles
    *(*kbase_gator_hwcnt_init_symbol) (struct kbase_gator_hwcnt_info *);
...
static void (*kbase_va_free_symbol) (struct kbase_context *,
				     struct kbase_hwc_dma_mapping *);
#endif

static mali_error(*kbase_instr_hwcnt_enable_symbol) (struct kbase_context *,
...
static mali_bool(*kbase_instr_hwcnt_dump_complete_symbol) (struct kbase_context
							   *, mali_bool *);
...
static const int number_of_hardware_counters =
ARRAY_SIZE(hardware_counter_names);
...
static int start(void)
{
	...
	for (cnt = 0; cnt < number_of_hardware_counters; cnt++) {
		if (counters[cnt].enabled) {
			...
			pr_debug
			    ("gator: %s: hardware counter %s selected [%d]\n",
	...
	if (num_hardware_counters_enabled > 0) {
		...
		if (kbcontext) {
			pr_err
			    ("gator: Mali-%s: error context already present\n",
			     mali_name);
			goto out;
		}
...
#elif MALI_DDK_GATOR_API_VERSION == 2
		kernel_dump_buffer =
		    kbase_va_alloc_symbol(kbcontext, 4096,
					  &kernel_dump_buffer_handle);
#endif
		...
		if (err != MALI_ERROR_NONE) {
			pr_err
			    ("gator: Mali-%s: can't setup hardware counters\n",
			     mali_name);
			goto free_buffer;
		}
...
#elif MALI_DDK_GATOR_API_VERSION == 2
		kbase_va_free_symbol(temp_kbcontext,
				     &kernel_dump_buffer_handle);
#endif
...
static int read_counter(const int cnt, const int len,
			const struct mali_counter *counter)
{
...
#if MALI_DDK_GATOR_API_VERSION == 3
	const char *block_base_address =
	    (char *)in_out_info->kernel_dump_buffer;
	int i;
	...
	for (i = 0; i < in_out_info->nr_hwc_blocks; i++) {
		...
		if (block_type != RESERVED_BLOCK) {
			if (counter_block == block_type) {
				value +=
				    *((u32 *) (block_base_address +
					       (0x100 * i)) + counter_offset);
	...
	const char *block_base_address =
	    (char *)kernel_dump_buffer + vithar_blocks[counter_block];
...
static int create_files(struct super_block *sb, struct dentry *root)
{
	...
	for (event = 0; event < number_of_hardware_counters; event++) {
		if (gator_mali_create_file_system
		    (gpu_core_name, hardware_counter_names[event], sb, root,
...
int gator_events_mali_midgard_hw_init(void)
{
	...
	uint32_t gpu_id = 0;
	const char *const *(*kbase_gator_hwcnt_init_names_symbol) (uint32_t *) =
	    NULL;
	...
	if (GPU_ID_IS_NEW_FORMAT(gpu_id)
	    && ((gpu_id >> GPU_ID2_ARCH_MAJOR_SHIFT) & 0xF) >= 6) {
	...
	hardware_counter_names =
	    kbase_gator_hwcnt_init_names_symbol(&number_of_hardware_counters);

	SYMBOL_CLEANUP(kbase_gator_hwcnt_init_names);

	if ((hardware_counter_names == NULL)
	    || (number_of_hardware_counters <= 0)) {
		pr_err
		    ("gator: Mali-%s: Error reading hardware counters names: got %d names\n",
	...
	counters =
	    kmalloc(sizeof(*counters) * number_of_hardware_counters,
		    GFP_KERNEL);
	counter_dump =
	    kmalloc(sizeof(*counter_dump) * number_of_hardware_counters * 2 + 4,
		    GFP_KERNEL);
