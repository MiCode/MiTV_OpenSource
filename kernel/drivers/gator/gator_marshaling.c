...
static void marshal_summary(long long timestamp, long long uptime,
			    long long monotonic_delta, const char *uname)
{
	unsigned long flags;
	int cpu = 0;
	char buf[32];
...
static bool marshal_cookie_header(const char *text)
{
	...
	return buffer_check_space(cpu, NAME_BUF,
				  strlen(text) + 3 * MAXSIZE_PACK32);
}
...
static void marshal_thread_name(int pid, char *name)
{
	...
	time = gator_get_time();
	if (buffer_check_space
	    (cpu, NAME_BUF,
	     TASK_COMM_LEN + 3 * MAXSIZE_PACK32 + MAXSIZE_PACK64)) {
		gator_buffer_write_packed_int(cpu, NAME_BUF,
					      MESSAGE_THREAD_NAME);
...
static void marshal_link(int cookie, int tgid, int pid)
{
	...
	time = gator_get_time();
	if (buffer_check_space
	    (cpu, ACTIVITY_BUF, MAXSIZE_PACK64 + 5 * MAXSIZE_PACK32)) {
...
static bool marshal_backtrace_header(int exec_cookie, int tgid, int pid,
				     u64 time)
{
	int cpu = get_physical_cpu();
	...
	if (!buffer_check_space
	    (cpu, BACKTRACE_BUF,
	     MAXSIZE_PACK64 + 5 * MAXSIZE_PACK32 +
	     gator_backtrace_depth * 2 * MAXSIZE_PACK32)) {
		/* Check and commit; commit is set to occur once buffer is 3/4 full */
...
static void marshal_backtrace_footer(u64 time)
{
	...
	gator_buffer_write_packed_int(cpu, BACKTRACE_BUF,
				      MESSAGE_END_BACKTRACE);
...
static bool marshal_event_header(u64 time)
{
	...
	local_irq_save(flags);
	if (buffer_check_space
	    (cpu, BLOCK_COUNTER_BUF, MAXSIZE_PACK32 + MAXSIZE_PACK64)) {
...
static void marshal_event(int len, int *buffer)
{
	...
	for (i = 0; i < len; i += 2) {
		if (!buffer_check_space
		    (cpu, BLOCK_COUNTER_BUF, 2 * MAXSIZE_PACK32))
			break;
		gator_buffer_write_packed_int(cpu, BLOCK_COUNTER_BUF,
					      buffer[i]);
		gator_buffer_write_packed_int(cpu, BLOCK_COUNTER_BUF,
					      buffer[i + 1]);
	}
...
static void marshal_event64(int len, long long *buffer64)
{
	...
	for (i = 0; i < len; i += 2) {
		if (!buffer_check_space
		    (cpu, BLOCK_COUNTER_BUF, 2 * MAXSIZE_PACK64))
			break;
		gator_buffer_write_packed_int64(cpu, BLOCK_COUNTER_BUF,
						buffer64[i]);
		gator_buffer_write_packed_int64(cpu, BLOCK_COUNTER_BUF,
						buffer64[i + 1]);
	}
...
static void __maybe_unused marshal_event_single(int core, int key, int value)
{
	...
	time = gator_get_time();
	if (buffer_check_space
	    (cpu, COUNTER_BUF, MAXSIZE_PACK64 + 3 * MAXSIZE_PACK32)) {
...
static void __maybe_unused marshal_event_single64(int core, int key,
						  long long value)
{
	unsigned long flags, cpu;
	u64 time;
	...
	time = gator_get_time();
	if (buffer_check_space
	    (cpu, COUNTER_BUF, 2 * MAXSIZE_PACK64 + 2 * MAXSIZE_PACK32)) {
		gator_buffer_write_packed_int64(cpu, COUNTER_BUF, time);
		gator_buffer_write_packed_int(cpu, COUNTER_BUF, core);
		gator_buffer_write_packed_int(cpu, COUNTER_BUF, key);
...
static void marshal_sched_trace_switch(int pid, int state)
{
	...
	time = gator_get_time();
	if (buffer_check_space
	    (cpu, SCHED_TRACE_BUF, MAXSIZE_PACK64 + 5 * MAXSIZE_PACK32)) {
		gator_buffer_write_packed_int(cpu, SCHED_TRACE_BUF,
					      MESSAGE_SCHED_SWITCH);
...
static void marshal_sched_trace_exit(int tgid, int pid)
{
	...
	time = gator_get_time();
	if (buffer_check_space
	    (cpu, SCHED_TRACE_BUF, MAXSIZE_PACK64 + 2 * MAXSIZE_PACK32)) {
		gator_buffer_write_packed_int(cpu, SCHED_TRACE_BUF,
					      MESSAGE_SCHED_EXIT);
		gator_buffer_write_packed_int64(cpu, SCHED_TRACE_BUF, time);
		gator_buffer_write_packed_int(cpu, SCHED_TRACE_BUF, pid);
...
static void marshal_idle(int core, int state)
{
	...
	time = gator_get_time();
	if (buffer_check_space
	    (cpu, IDLE_BUF, MAXSIZE_PACK64 + 2 * MAXSIZE_PACK32)) {
...
static void marshal_core_name(const int core, const int cpuid, const char *name)
{
	...
	local_irq_save(flags);
	if (buffer_check_space
	    (cpu, SUMMARY_BUF, MAXSIZE_PACK32 + MAXSIZE_CORE_NAME)) {
		gator_buffer_write_packed_int(cpu, SUMMARY_BUF,
					      MESSAGE_CORE_NAME);
...
static void marshal_activity_switch(int core, int key, int activity, int pid,
				    int state)
{
	unsigned long cpu = get_physical_cpu(), flags;
	u64 time;
	...
	if (buffer_check_space
	    (cpu, ACTIVITY_BUF, MAXSIZE_PACK64 + 5 * MAXSIZE_PACK32)) {
		gator_buffer_write_packed_int(cpu, ACTIVITY_BUF,
					      MESSAGE_SWITCH);
		gator_buffer_write_packed_int64(cpu, ACTIVITY_BUF, time);
