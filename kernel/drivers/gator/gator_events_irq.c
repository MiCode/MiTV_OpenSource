...
static int gator_events_irq_create_files(struct super_block *sb,
					 struct dentry *root)
{
	struct dentry *dir;
	int i;
	...
	/* soft irq */
	for (i = 0; i < gator_cluster_count; i++) {
		snprintf(buf, sizeof(buf), "%s_softirq",
			 gator_clusters[i]->pmnc_name);
		dir = gatorfs_mkdir(sb, root, buf);
		if (!dir)
			return -1;
...
static int gator_events_irq_start(void)
{
 ...
 fail_hardirq_exit:
	pr_err
	    ("gator: irq tracepoints failed to activate, please verify that tracepoints are enabled in the linux kernel\n");
