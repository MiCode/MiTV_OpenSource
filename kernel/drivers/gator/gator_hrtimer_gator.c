...
static enum hrtimer_restart gator_hrtimer_notify(struct hrtimer *hrtimer)
{
	...
	hrtimer_forward(hrtimer, per_cpu(hrtimer_expire, cpu),
			profiling_interval);
	per_cpu(hrtimer_expire, cpu) =
	    ktime_add(per_cpu(hrtimer_expire, cpu), profiling_interval);
	(*callback) ();
...
static void gator_hrtimer_online(void)
{
	...
	if (per_cpu(hrtimer_is_active, cpu)
	    || (ktime_to_ns(profiling_interval) == 0))
		return;
...
#endif
	per_cpu(hrtimer_expire, cpu) =
	    ktime_add(hrtimer->base->get_time(), profiling_interval);
	hrtimer_start(hrtimer, per_cpu(hrtimer_expire, cpu),
