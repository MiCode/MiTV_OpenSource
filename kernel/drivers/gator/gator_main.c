...
static int gator_trace_power_create_files(struct super_block *sb,
					  struct dentry *root);
static int sched_trace_create_files(struct super_block *sb,
				    struct dentry *root);
 ...
 * Commit interface
 ******************************************************************************/
static bool buffer_commit_ready(int prev_cpu, int prev_buftype, int *out_cpu,
				int *out_buftype)
{
	int cpu_x, x;
	...
	for (x = 0; x < NUM_GATOR_BUFS; x++) {
		for_each_present_cpu(cpu_x) {
			if ((cpu_x > prev_cpu)
			    || ((cpu_x == prev_cpu) && (x > prev_buftype))) {
				if (per_cpu(gator_buffer_commit, cpu_x)[x] !=
				    per_cpu(gator_buffer_read, cpu_x)[x]) {
					*out_cpu = cpu_x;
					*out_buftype = x;
					return true;
				}
...
static void gator_send_core_name(const int cpu, const u32 cpuid)
{
	...
	if (!sent_core_name[cpu] || (cpuid != gator_cpuids[cpu])) {
		const struct gator_cpu *const gator_cpu =
		    gator_find_cpu_by_cpuid(cpuid);
		...
		if (gator_cpu != NULL) {
			...
			if (cpuid == -1)
				snprintf(core_name_buf, sizeof(core_name_buf),
					 "Unknown");
			else
				snprintf(core_name_buf, sizeof(core_name_buf),
					 "Unknown (0x%.5x)", cpuid);
...
u64 gator_get_time(void)
{
	...
	if (prev_timestamp <= timestamp) {
		...
		if (!printed_monotonic_warning && delta > 500000) {
			pr_err
			    ("%s: getrawmonotonic is not monotonic  cpu: %i  delta: %lli\nSkew in Streamline data may be present at the fine zoom levels\n",
			     __func__, cpu, delta);
...
static void gator_emit_perf_time(u64 time)
{
	...
	if (time >= gator_sync_time) {
		...
		if (gator_live_rate <= 0)
			gator_commit_buffer(get_physical_cpu(), COUNTER_BUF,
					    time);
	}
#endif
}
...
static int gator_register_hotcpu_notifier(void)
{
	...
	retval =
	    cpuhp_setup_state(CPUHP_AP_ONLINE_DYN, "gator/cpuhotplug:online",
			      gator_cpuhp_notify_online,
			      gator_cpuhp_notify_offline);
	if (retval >= 0) {
...
static int gator_hotcpu_notify(struct notifier_block *self,
 ...
 * continue to collect
 */
static int gator_pm_notify(struct notifier_block *nb, unsigned long event,
			   void *dummy)
{
	int cpu;
	struct timespec ts;
	...
	case PM_POST_SUSPEND:
		...
		if (gator_hibernate_time > 0) {
			getnstimeofday(&ts);
			gator_monotonic_started +=
			    gator_hibernate_time + gator_get_time() -
			    timespec_to_ns(&ts);
...
static int gator_start(void)
{
	...
	gator_buffer_wake_thread =
	    kthread_run(gator_buffer_wake_func, NULL, "gator_bwake");
...
static int gator_op_setup(void)
{
	...
	for (i = 0; i < NUM_GATOR_BUFS; i++) {
		...
		for_each_present_cpu(cpu) {
			...
			per_cpu(gator_buffer_commit_time, cpu) =
			    gator_live_rate;
			...
			per_cpu(gator_buffer, cpu)[i] =
			    vmalloc(gator_buffer_size[i]);
...
static ssize_t enable_read(struct file *file, char __user * buf, size_t count,
			   loff_t * offset)
{
				     ...
				     size_t count, loff_t * offset)
{
	 ...
	 */
	wait_event_interruptible(gator_buffer_wait,
				 buffer_commit_ready(-1, -1, &cpu, &buftype)
				 || !gator_started);
	...
	do {
		...
		/* start, middle or end */
		if (length1 > 0
		    && copy_to_user(&buf[written], buffer1, length1))
			break;

		/* possible wrap around */
		if (length2 > 0
		    && copy_to_user(&buf[written + length1], buffer2, length2))
			break;
...
static ssize_t depth_write(struct file *file, char const __user * buf,
			   size_t count, loff_t * offset)
{
	unsigned long val;
	int retval;
...
static void gator_op_create_files(struct super_block *sb, struct dentry *root)
{
	...
	gatorfs_create_ro_ulong(sb, root, "buffer_size",
				&userspace_buffer_size);
...
int gator_new_tracepoint_module(struct notifier_block *nb, unsigned long action,
				void *data)
{
	struct tp_module *tp_mod = (struct tp_module *)data;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
	tracepoint_ptr_t *begin = tp_mod->mod->tracepoints_ptrs;
	tracepoint_ptr_t *end =
	    tp_mod->mod->tracepoints_ptrs + tp_mod->mod->num_tracepoints;
#else
	...
	if (action == MODULE_STATE_COMING) {
		for (; begin != end; ++begin) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			gator_save_tracepoint(tracepoint_ptr_deref(begin),
					      NULL);
#else
		...
		for (; begin != end; ++begin) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			gator_unsave_tracepoint(tracepoint_ptr_deref(begin),
						NULL);
#else
		...
		pr_debug
		    ("gator: unexpected action value in gator_new_tracepoint_module: 0x%lx\n",
		     action);
	}
...
MODULE_PARM_DESC(disable_cpu_onlining,
		 "Do not online all cores when module starts");
