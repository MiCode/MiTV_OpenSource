...
__maybe_unused
    static const struct gator_cpu *gator_find_cpu_by_pmu_name(const char *const
							      name)
{
	const struct gator_cpu *gator_cpu;
	...
	list_for_each_entry(gator_cpu, &gator_cpus, list) {
		if (gator_cpu->pmnc_name != NULL &&
		    ...
		    (strcasecmp(gator_cpu->pmnc_name, name) == 0 ||
		     ...
		     ((strncasecmp
		       (name, OLD_PMU_PREFIX, sizeof(OLD_PMU_PREFIX) - 1) == 0
		       && strncasecmp(gator_cpu->pmnc_name, NEW_PMU_PREFIX,
				      sizeof(NEW_PMU_PREFIX) - 1) == 0
...
__maybe_unused
    static const struct uncore_pmu *gator_find_uncore_pmu(const char *const
							  name)
{
	const struct uncore_pmu *uncore_pmu;

	list_for_each_entry(uncore_pmu, &uncore_pmus, list) {
		if (uncore_pmu->pmnc_name != NULL
		    && strcasecmp(uncore_pmu->pmnc_name, name) == 0)
			return uncore_pmu;
	}
...
static ssize_t gator_pmu_init_write(struct file *file, char const __user * buf,
				    size_t count, loff_t * offset)
{
	...
	/* cluster information */
	{
		...
		dir =
		    gatorfs_mkdir(gator_sb, file->f_path.dentry->d_parent,
				  "clusters");
		for (i = 0; i < gator_cluster_count; i++) {
			gator_cluster_ids[i] = i;
			gatorfs_create_ro_ulong(gator_sb, dir,
						gator_clusters[i]->pmnc_name,
						&gator_cluster_ids[i]);
		}
	}
...
static ssize_t gator_pmu_str_read_file(struct file *file, char __user * buf,
				       size_t count, loff_t * offset)
{
...
static ssize_t gator_pmu_str_write_file(struct file *file,
...
static int gator_pmu_create_str(struct super_block *sb, struct dentry *root,
				char const *name, char *const val)
{
	struct dentry *d =
	    __gatorfs_create_file(sb, root, name, &gator_pmu_str_fops, 0644);
	if (!d)
		return -EFAULT;
...
static ssize_t gator_pmu_cpumask_read(struct file *file, char __user * buf,
				      size_t count, loff_t * offset)
{
	...
	cpumask =
	    (struct cpumask *)GATOR_ATOMIC_READ(&(uncore_pmu->cpumask_atomic));
	if (!cpumask)
		return simple_read_from_buffer(buf, count, offset,
					       GATOR_NONE_STRING,
					       strlen(GATOR_NONE_STRING));
	...
	return simple_read_from_buffer(buf, count, offset, buffer,
				       strlen(buffer));
}

static ssize_t gator_pmu_cpumask_write(struct file *file,
				       char const __user * ubuf, size_t count,
				       loff_t * offset)
{
	struct uncore_pmu *uncore_pmu = NULL;
	struct cpumask *cpumask = NULL;
	...
	if (!cpumask) {
		...
		ret =
		    (struct cpumask *)
		    GATOR_ATOMIC_CMPXCHG(&(uncore_pmu->cpumask_atomic),
					 GATOR_ATOMIC_PTR_TO_COUNTER(NULL),
					 GATOR_ATOMIC_PTR_TO_COUNTER(cpumask));
		if (ret != NULL) {
...
static ssize_t gator_pmu_export_write(struct file *file,
				      char const __user * ubuf, size_t count,
				      loff_t * offset)
{
	struct dentry *dir;
	...
	if (strcmp("pmu", parent->d_name.name) == 0) {
		...
		gator_pmu_create_str(gator_sb, dir, "core_name",
				     gator_cpu->core_name);
		strcpy(gator_cpu->pmnc_name, str);
		gator_pmu_create_str(gator_sb, dir, "dt_name",
				     gator_cpu->dt_name);
		gatorfs_create_ulong(gator_sb, dir, "pmnc_counters",
				     &gator_cpu->pmnc_counters);
		...
		gatorfs_create_ulong(gator_sb, dir, "pmnc_counters",
				     &uncore_pmu->pmnc_counters);
		gatorfs_create_ulong(gator_sb, dir, "has_cycles_counter",
				     &uncore_pmu->has_cycles_counter);
		gatorfs_create_file_data(gator_sb, dir, "cpumask",
...
static int gator_pmu_create_files(struct super_block *sb, struct dentry *root,
	...
	}
	{
		...
		list_for_each_entry_safe(uncore_pmu, next, &uncore_pmus, list) {
			struct cpumask *cpumask =
			    (struct cpumask *)
			    GATOR_ATOMIC_READ(&(uncore_pmu->cpumask_atomic));
			if (cpumask) {
				kfree(cpumask);
			}
