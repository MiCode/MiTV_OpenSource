...
GATOR_DEFINE_PROBE(mm_page_free,
...
GATOR_DEFINE_PROBE(mm_page_alloc,
		   TP_PROTO(struct page *page, unsigned int order,
...
static int gator_events_meminfo_create_files(struct super_block *sb,
					     struct dentry *root)
{
	struct dentry *dir;
	int i;
...
#else
	setup_deferrable_timer_on_stack(&meminfo_wake_up_timer,
					meminfo_wake_up_handler, 0);
#endif
			...
			case MEMINFO_CACHED:
				...
				value =
				    (READ_NR_FILE_PAGES
				     ...
				     info.bufferram) * PAGE_SIZE;
				break;
			case MEMINFO_SLAB:
				value =
				    (READ_NR_SLAB_RECLAIMABLE +
				     READ_NR_SLAB_UNRECLAIMABLE) * PAGE_SIZE;
				break;
			default:
				value = 0;
				break;
...
static int gator_events_meminfo_read_proc(long long **buffer,
...
#else
		share =
		    get_mm_counter(mm, MM_FILEPAGES) + get_mm_counter(mm,
								      MM_SHMEMPAGES);
#endif
	}
