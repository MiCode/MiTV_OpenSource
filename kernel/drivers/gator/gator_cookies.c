...
static uint32_t get_cookie(int cpu, struct task_struct *task, const char *text,
			   bool from_wq);
...
static uint32_t gator_chksum_crc32(const char *data)
{
	...
	for (i = 0; i < length; i++)
		crc =
		    ((crc >> 8) & 0x00FFFFFF) ^
		    gator_crc32_table[(crc ^ *block++) & 0xFF];
...
static void translate_buffer_write_args(int cpu, struct task_struct *task,
					const char *text)
{
	unsigned long flags;
	int write;
...
static void translate_buffer_read_args(int cpu, struct cookie_args *args)
{
	...
	*args = per_cpu(translate_buffer, cpu)[read];
	per_cpu(translate_buffer_read, cpu) =
	    (read + 1) & translate_buffer_mask;
...
static void wq_cookie_handler(struct work_struct *unused)
{
	...
	if (gator_started != 0) {
		while (per_cpu(translate_buffer_read, cpu) !=
		       per_cpu(translate_buffer_write, cpu)) {
...
static int translate_app_process(const char **text, int cpu,
				 struct task_struct *task, bool from_wq)
{
	void *maddr;
	unsigned int len;
	unsigned long addr;
	...
	while (len) {
		if (get_user_pages_remote
		    (task, mm, addr, 1, 0, 1, &page, &page_vma) <= 0)
			goto outsem;
		...
		copy_from_user_page(page_vma, page, addr, buf, maddr + offset,
				    bytes);
	...
	if (strcmp(*text, "zygote") == 0
	    || strcmp(*text, "<pre-initialized>") == 0)
		retval = 0;
...
static int get_exec_cookie(int cpu, struct task_struct *task)
{
	...
	if (task && task->mm && task->mm->exe_file) {
		text =
		    d_path(&task->mm->exe_file->f_path,
			   per_cpu(scratch, get_physical_cpu()), PAGE_SIZE);
...
static unsigned long get_address_cookie(int cpu, struct task_struct *task,
					unsigned long addr, off_t * offset)
{
...
static int cookies_initialize(void)
{
	...
	translate_buffer_mask =
	    TRANSLATE_BUFFER_SIZE / sizeof(per_cpu(translate_buffer, 0)[0]) - 1;
	...
	for_each_present_cpu(cpu) {
		...
		per_cpu(translate_buffer, cpu) =
		    kmalloc(TRANSLATE_BUFFER_SIZE, GFP_KERNEL);
		...
		per_cpu(translate_text, cpu) =
		    kmalloc(TRANSLATE_TEXT_SIZE, GFP_KERNEL);
	...
	poly = 0x04c11db7;
	gator_crc32_table =
	    kmalloc(256 * sizeof(*gator_crc32_table), GFP_KERNEL);
	...
	setup_deferrable_timer_on_stack(&app_process_wake_up_timer,
					app_process_wake_up_handler, 0);
