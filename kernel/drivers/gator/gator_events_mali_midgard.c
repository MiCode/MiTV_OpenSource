 ...
 */
static inline long get_duration_us(const struct timespec *start,
				  ...
				  unsigned int type)
{
	...
	struct timespec *event_start =
	    &timeline_event_starttime[timeline_index];
	...
	case ACTIVITY_STOP:
		...
		if (event_start->tv_sec != 0) {
			...
			timeline_data[timeline_index] +=
			    get_duration_us(event_start, &event_timestamp);
...
GATOR_DEFINE_PROBE(mali_pm_status,
	...
	case TILER_PRESENT_LO:
		{
			...
			if (BIT_AT(changed, 0))
				record_timeline_event(PM_TILER_0,
						      BIT_AT(value,
							     0) ? ACTIVITY_START
						      : ACTIVITY_STOP);
	...
	case L2_PRESENT_LO:
		{
			...
			if (BIT_AT(changed, 0))
				record_timeline_event(PM_L2_0,
						      BIT_AT(value,
							     0) ? ACTIVITY_START
						      : ACTIVITY_STOP);
			if (BIT_AT(changed, 4))
				record_timeline_event(PM_L2_1,
						      BIT_AT(value,
							     4) ? ACTIVITY_START
						      : ACTIVITY_STOP);
...
GATOR_DEFINE_PROBE(mali_page_fault_insert_pages,
...
GATOR_DEFINE_PROBE(mali_total_alloc_pages_change,
...
static int create_files(struct super_block *sb, struct dentry *root)
{
	...
	for (event = FIRST_TIMELINE_EVENT;
	     event < FIRST_TIMELINE_EVENT + NUMBER_OF_TIMELINE_EVENTS;
	     event++) {
		if (gator_mali_create_file_system
		    (mali_name, timeline_event_names[counter_index], sb, root,
		     &counters[event], NULL) != 0)
			return -1;
		counter_index++;
	}
	counter_index = 0;
	for (event = FIRST_SOFTWARE_COUNTER;
	     event < FIRST_SOFTWARE_COUNTER + NUMBER_OF_SOFTWARE_COUNTERS;
	     event++) {
		if (gator_mali_create_file_system
		    (mali_name, software_counter_names[counter_index], sb, root,
		     &counters[event], NULL) != 0)
			return -1;
		counter_index++;
	}
	counter_index = 0;
	for (event = FIRST_ACCUMULATOR;
	     event < FIRST_ACCUMULATOR + NUMBER_OF_ACCUMULATORS; event++) {
		if (gator_mali_create_file_system
		    (mali_name, accumulators_names[counter_index], sb, root,
		     &counters[event], NULL) != 0)
			return -1;
		counter_index++;
	}
	...
	if (mali_control) {
		if (gator_mali_create_file_system
		    (mali_name, "Filmstrip_cnt0", sb, root,
		     &counters[FILMSTRIP], &filmstrip_event) != 0)
			return -1;
...
static int register_tracepoints(void)
{
	if (GATOR_REGISTER_TRACE(mali_pm_status)) {
		pr_err
		    ("gator: %s: mali_pm_status tracepoint failed to activate\n",
		     mali_name);
		return 0;
	}
	...
	if (GATOR_REGISTER_TRACE(mali_page_fault_insert_pages)) {
		pr_err
		    ("gator: %s: mali_page_fault_insert_pages tracepoint failed to activate\n",
		     mali_name);
		return 0;
	}
	...
	if (GATOR_REGISTER_TRACE(mali_mmu_as_in_use)) {
		pr_err
		    ("gator: %s: mali_mmu_as_in_use tracepoint failed to activate\n",
		     mali_name);
		return 0;
	}
	...
	if (GATOR_REGISTER_TRACE(mali_mmu_as_released)) {
		pr_err
		    ("gator: %s: mali_mmu_as_released tracepoint failed to activate\n",
		     mali_name);
		return 0;
	}
	...
	if (GATOR_REGISTER_TRACE(mali_total_alloc_pages_change)) {
		pr_err
		    ("gator: %s: mali_total_alloc_pages_change tracepoint failed to activate\n",
		     mali_name);
		return 0;
	}
...
static int start(void)
{
	...
	for (cnt = 0; cnt < NUMBER_OF_TIMELINE_EVENTS; cnt++) {
		timeline_event_starttime[cnt].tv_sec =
		    timeline_event_starttime[cnt].tv_nsec = 0;
		timeline_data[cnt] = 0;
	}
...
#define FBDUMP_CONTROL_RESIZE_FACTOR (4)
		...
		pr_debug
		    ("gator: sent mali_control enabled=%d, rate=%d, resize_factor=%d\n",
...
static void stop(void)
{
	...
	pr_debug
	    ("gator: %s: mali_page_fault_insert_pages tracepoint deactivated\n",
	     mali_name);
	...
	pr_debug
	    ("gator: %s: mali_total_alloc_pages_change tracepoint deactivated\n",
	     mali_name);
...
static int read(int **buffer, bool sched_switch)
{
	...
	if (prev_timestamp.tv_sec != 0)
		sample_interval_us =
		    get_duration_us(&prev_timestamp, &read_timestamp);
	 ...
	 */
	for (cnt = FIRST_TIMELINE_EVENT;
	     cnt < (FIRST_TIMELINE_EVENT + NUMBER_OF_TIMELINE_EVENTS); cnt++) {
		...
		if (counter->enabled) {
			...
			if (timeline_event_starttime[index].tv_sec != 0) {
				const long event_duration =
				    get_duration_us(&timeline_event_starttime
						    [index], &read_timestamp);
	...
	for (cnt = FIRST_SOFTWARE_COUNTER;
	     cnt < (FIRST_SOFTWARE_COUNTER + NUMBER_OF_SOFTWARE_COUNTERS);
	     cnt++) {
	...
	for (cnt = FIRST_ACCUMULATOR;
	     cnt < (FIRST_ACCUMULATOR + NUMBER_OF_ACCUMULATORS); cnt++) {
		const struct mali_counter *counter = &counters[cnt];
