...
struct gator_kmod_stack {
	unsigned long addresses[GATOR_KMOD_STACK_MAX_SIZE];
};
...
#else
	trace.max_entries =
	    (kernel_stack_unwinding ? gator_backtrace_depth : 1);
#endif
...
static void gator_add_trace(int cpu, unsigned long address)
{
	...
	unsigned long cookie =
	    get_address_cookie(cpu, current, address & ~1, &offset);
...
static void arm_backtrace_eabi(int cpu, struct pt_regs *const regs,
			       unsigned int depth)
{
	...
	while (depth-- && curr) {
		if (!access_ok
		    (VERIFY_READ, curr, sizeof(struct stack_frame_eabi))
		    || __copy_from_user_inatomic(&bufcurr, curr,
						 sizeof(struct
							stack_frame_eabi))) {
			return;
		}
...
static void gator_add_sample(int cpu, struct pt_regs *const regs, u64 time)
{
	...
	if (!marshal_backtrace_header
	    (exec_cookie, current->tgid, current->pid, time))
		return;
