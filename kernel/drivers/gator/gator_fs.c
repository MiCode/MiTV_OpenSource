...
static ssize_t gatorfs_ulong_to_user(unsigned long val, char __user * buf,
...
static ssize_t gatorfs_u64_to_user(u64 val, char __user * buf, size_t count,
				   loff_t * offset)
{
	char tmpbuf[TMPBUFSIZE];
...
static int gatorfs_ulong_from_user(unsigned long *val, char const __user * buf,
				   size_t count)
{
	char tmpbuf[TMPBUFSIZE];
	unsigned long flags;
...
static int gatorfs_u64_from_user(u64 * val, char const __user * buf,
				 size_t count)
{
	char tmpbuf[TMPBUFSIZE];
	unsigned long flags;
...
static ssize_t ulong_read_file(struct file *file, char __user * buf,
			       size_t count, loff_t * offset)
{
...
static ssize_t u64_read_file(struct file *file, char __user * buf, size_t count,
			     loff_t * offset)
{
...
static ssize_t u64_write_file(struct file *file, char const __user * buf,
			      size_t count, loff_t * offset)
{
...
static ssize_t atomic_read_file(struct file *file, char __user * buf,
				size_t count, loff_t * offset)
{
...
{
	struct dentry *dentry =
	    __gatorfs_create_file(sb, root, name, fops, 0644);
	if (!dentry)
		return -EFAULT;
