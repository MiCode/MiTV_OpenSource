...
static void marshal_frame(int cpu, int buftype)
{
	...
	if (gator_response_type > 0)
		gator_buffer_write_packed_int(cpu, buftype,
					      gator_response_type);

	/* leave space for 4-byte unpacked length */
	per_cpu(gator_buffer_write, cpu)[buftype] =
	    (per_cpu(gator_buffer_write, cpu)[buftype] +
	     sizeof(s32)) & gator_buffer_mask[buftype];
...
static int buffer_bytes_available(int cpu, int buftype)
{
	...
	filled =
	    per_cpu(gator_buffer_write,
		    cpu)[buftype] - per_cpu(gator_buffer_read, cpu)[buftype];
	if (filled < 0)
...
static int contiguous_space_available(int cpu, int buftype)
{
	int remaining = buffer_bytes_available(cpu, buftype);
	int contiguous =
	    gator_buffer_size[buftype] - per_cpu(gator_buffer_write,
						 cpu)[buftype];
...
static void gator_commit_buffer(int cpu, int buftype, u64 time)
{
	...
	for (byte = 0; byte < sizeof(s32); byte++)
		per_cpu(gator_buffer,
			cpu)[buftype][(commit + type_length +
				       byte) & gator_buffer_mask[buftype]] =
		    (length >> byte * 8) & 0xFF;

	per_cpu(gator_buffer_commit, cpu)[buftype] =
	    per_cpu(gator_buffer_write, cpu)[buftype];
...
static void buffer_check(int cpu, int buftype, u64 time)
{
	int filled =
	    per_cpu(gator_buffer_write,
		    cpu)[buftype] - per_cpu(gator_buffer_commit, cpu)[buftype];
