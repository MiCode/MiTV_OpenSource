...
repeat:
    ...
    while (count++ < 10)
    {
        ...
        if ((usr & MSTAR_UART_BUSY) ==0 && (usr & MSTAR_UART_RX_BUSY) == 0)
        {
			...
			UART_PRINT("Update UART divisor:0x%x\n", pre_divisor);
            bRet = 1;
            break;
        }
    }
    ...
    if (count == 10)
        UART_PRINT(KERN_ERR"Uart is too busy to change baud rate: 0x%x !!\n", divisor);
...
static irqreturn_t serial8250_interrupt(int irq, void *dev_id)
{
	...
	do {
        ...
        serial_port_out(port, UART_IER, 0x07);
		...
		else if (iir != UART_IIR_NO_INT)
		{
			...
			serial_in(up, MSTAR_UART_USR);
            serial_in(up, MSTAR_UART_USR+1);
			handled = 1;
			end = NULL;
		}
        #endif
		...
		if (l == i->head && pass_counter++ > PASS_LIMIT) {
            ...
            if(printk_ratelimit())
			    UART_PRINT(KERN_EMERG "serial8250: uart port%d, too much work usr:0x%x 0x%x\n",
			        port->line, serial_in(up, MSTAR_UART_USR),  serial_in(up, MSTAR_UART_USR+1));
 #if DYNAMIC_BAUDRATE_CHANGE_ENABLE
			hang_counter++;
			if (hang_counter == 10)
			{
				hang_counter =0;
				UART_PRINT(KERN_EMERG "serial8250: recovery uart1...\n");
				UART16550_Init(up);
			}
#endif
			break;
...
static int univ8250_setup_irq(struct uart_8250_port *up)
{
	...
	if (up->bugs & UART_BUG_THRE) {
		UART_DEBUG("%s - using backup timer\n", port->name);
...
void serial8250_resume_port(int line)
{
    ...
    {
       UART_PRINT("[BUG][PM_SLP]:uart_rx_enable is modified by others\n");
...
#define CONFIG_STR_UART_RX_DISABLE 1
#ifdef CONFIG_STR_UART_RX_DISABLE
#define CHIPTOP_PMSLP RIU_VIRT_BASE+0x1C00
#define PM_SLP_UART 0x9
#define PM_SLP_UART_MASK 0xC00
static unsigned short reg_pm_slp_uart_onoff;
#endif
...
#ifdef CONFIG_STR_UART_RX_DISABLE
    //pr_emerg("Do serial8250_suspend.\n");
	reg_pm_slp_uart_onoff = UART_REG(CHIPTOP_PMSLP, PM_SLP_UART) & PM_SLP_UART_MASK;
#endif
	for (i = 0; i < UART_NR; i++) {
		struct uart_8250_port *up = &serial8250_ports[i];
...
#ifdef CONFIG_STR_UART_RX_DISABLE
	UART_REG(CHIPTOP_PMSLP, PM_SLP_UART) |= reg_pm_slp_uart_onoff;
#endif
