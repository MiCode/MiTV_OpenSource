...
static int __btpwrkey __attribute__((__aligned__(8)));
core_param(btpwrkey, __btpwrkey, int, 0644);
extern u32 powerkey[];
			  ...
			  unsigned int type, unsigned int code, int *pval)
{
	...
	case EV_KEY:
		if (is_event_supported(code, dev->keybit, KEY_MAX)) {
			...
			/* Update wakeup reason on all ir/keypad/btrc keycode.
			TODO: device(vid/pid) and keycode should be matched. */
	...
	case EV_MSC:
		...
		if (is_event_supported(code, dev->mscbit, MSC_MAX)) {
			...
			if (__btpwrkey && (disposition & INPUT_PASS_TO_HANDLERS) &&
					dev->id.bustype == BUS_BLUETOOTH &&
					(value & 0xffff) == __btpwrkey) {
				pr_info("%s: BT PWR key usage matched!(0x%04x, 0x%04x)\n",
					__func__, code, __btpwrkey);
				mstar_cust_pwrkey_handler(powerkey[0]);
			}
#endif // BT_RC_NTS_WAKEUP
#endif
		}
		pr_info("%s:EV_MSC btntskey=0x%x code=0x%x value=0x%0x bus=%d en_blk_key=%d disposition=%d\n",
			__func__, __btntskey, code, value, dev->id.bustype, en_blk_key, disposition);
		break;
