...
printStatus() {
	...
	if [ $FTRACECHECK != "yes" ]; then
		...
		echo "trace events {"
		files="suspend_resume device_pm_callback_end device_pm_callback_start utpa_pm_callback_start utpa_pm_callback_end"
		for f in $files; do
			if [ -e "$EPATH/$f" ]; then
				echo "    $f: FOUND"
			else
...
suspendPrepare() {
	...
	echo -n "."
	writeToSysFile "$EPATH/utpa_pm_callback_start/enable" "1"
	echo -n "."
	writeToSysFile "$EPATH/utpa_pm_callback_end/enable" "1"
	echo -n "."
