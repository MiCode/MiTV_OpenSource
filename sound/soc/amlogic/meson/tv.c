...
static int aml_mute_unmute(struct snd_soc_card *card,
			int av_mute, int amp_mute);

/* xiaomi add start */
static const char *const speaker_channel_mask_texts[] = {
	"0/1", "2/3", "4/5", "6/7" };

static const struct soc_enum speaker0_channel_mask_enum =
	SOC_ENUM_SINGLE(SND_SOC_NOPM, 0, ARRAY_SIZE(speaker_channel_mask_texts),
			speaker_channel_mask_texts);

static int aml_audio_get_speaker0_channel_mask(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card =  snd_kcontrol_chip(kcontrol);
	struct aml_audio_private_data *p_aml_audio =
			snd_soc_card_get_drvdata(card);
	int value = p_aml_audio->Speaker0_Channel_Mask;
...
/* xiaomi add start */
	SOC_ENUM_EXT("Speaker0 Channel Mask",
		     speaker0_channel_mask_enum,
		     aml_audio_get_speaker0_channel_mask,
		     NULL),
/* xiaomi add end */
...
static int aml_get_audio_hp_mute(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct aml_audio_private_data *p_aml_audio;
	...
	ucontrol->value.integer.value[0] = p_aml_audio->hp_mute;
	return 0;
}

static int aml_set_audio_hp_mute(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct aml_audio_private_data *p_aml_audio
		= snd_soc_card_get_drvdata(card);
	int hp_mute = ucontrol->value.integer.value[0];
...
static const struct snd_kcontrol_new hp_controls[] = {
	   SOC_ENUM_EXT("Hp Status",
			   ...
			   NULL),
	   SOC_SINGLE_BOOL_EXT("Hp Mute",
			   0,
			   aml_get_audio_hp_mute,
			   aml_set_audio_hp_mute),
};
...
#ifdef HP_EXTCON
			extcon_set_state_sync(p_aml_audio->hp_extcon_dev,
				EXTCON_JACK_HEADPHONE, true);
#endif
			audio_hp_status = 1;
			//When hp pluged, mute av and spk.
			aml_mute_unmute(card, 0, 1);
		} else {
...
#ifdef HP_EXTCON
static void audio_extcon_register(struct platform_device *pdev)
{
	...
	if (IS_ERR(edev)) {
		pr_info("failed to allocate extcon h2w\n");
		return;
	}
	//edev->dev.parent = &pdev->dev;
	edev->dev.parent = NULL;
	edev->name = "h2w";
	dev_set_name(&edev->dev, "h2w");
	ret = extcon_dev_register(edev);
	if (ret < 0) {
		pr_info("failed to register extcon h2w\n");
		return;
	}
	p_aml_audio->hp_extcon_dev = edev;
}
#endif
...
struct class_compat *speaker_ctrl_class;
static int xiaomi_create_speaker_class(void)
{
	int retval = 0;

	speaker_ctrl_class = class_compat_register("xiaomi-speaker");
	if (!speaker_ctrl_class) {
		retval = -ENOMEM;
		pr_err("error: failed to create class 'speaker' %d\n",
			retval);
	}
...
{
	return speaker_ctrl_class;
}
EXPORT_SYMBOL(xiaomi_get_speaker_class);
#endif

static int xiaomi_spk_type = XIAOMI_SPK_TYPE_MAX;

static int __init speaker_select(char *str)
{
	char __speaker[32] = "10w";

	if (str != NULL) {
		snprintf(__speaker, sizeof(__speaker), "%s", str);
		if (!strncmp(__speaker, "10w", 3))
			xiaomi_spk_type = XIAOMI_SPK_TYPE_10W;
		else if (!strncmp(__speaker, "8w", 2))
			xiaomi_spk_type = XIAOMI_SPK_TYPE_08W;
		else
			xiaomi_spk_type = XIAOMI_SPK_TYPE_MAX;
		pr_info("xiaomi, select speaker type: %s\n", __speaker);
	} else {
		pr_info("xiaomi, select speaker type is NULL\n");
	}
	return 0;
}

__setup("speaker=", speaker_select);

int xiaomi_get_speaker_type(void)
{
	return xiaomi_spk_type;
}
EXPORT_SYMBOL(xiaomi_get_speaker_type);
