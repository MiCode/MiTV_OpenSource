					...
					int **result)
{
...
#endif
	// migratepage is the page need to be migrated @ migrate_list(List of pages being migrated)
	...
	if (list_empty(&cc->freepages)) {
		if (!cc->contended)
			isolate_freepages(cc);	// here to get cc->freepages, pages @ cc->freepages are isolated, free, pages
	...
	if(is_cma_page(freepage)) {
		if((mapping = page_mapping(migratepage))) {
			if(!(mapping_gfp_mask(mapping) & __GFP_MOVABLE)) {
				return migratepage;	// unmovable to movable
			}
		} else {
			return migratepage;	// file_cache to cma
		}
	}
#endif
