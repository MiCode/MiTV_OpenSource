...
static void __adjust_managed_cma_page_count(struct zone *zone, long count)
{
    ...
    if (!CMA_set_threshold_done && lowmem_minfree[lowmem_minfree_index])
	{
		...
		printk(KERN_NOTICE "\033[31mrealtotalpages is %u pages\033[m\n", realtotalpages);
...
out:
	...
	if(dcma_debug)
		printk(KERN_EMERG "\033[31mlx_free is %d kB, cma is %d kB\033[m\n", (lx_free << (PAGE_SHIFT - 10)), (cma  << (PAGE_SHIFT - 10)));
#endif
}
...
int read_CMA_threshold_proc(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
	show_dcma_info();
	return 0;
}

int write_CMA_threshold_proc(struct file *file, const char *buf, int count, void *data)
{
    char *th1, *th2;
    char* cur = proc_data;
    ...
    if((th1 == NULL) || (th2 == NULL)){
        printk("TH1 or TH2 == NULL\n");
        return count;
    }
    if((th1[0] == '\0') || (th2[0] == '\0')){
        printk("TH1 or TH2 == empty value\n");
        return count;
    }

    if(kstrtouint(th1, 10, &val_th1))
	{
		printk("invalid config th1 = %s, kstrtouint error\n", th1);
		return -EINVAL;
	}
    if(kstrtouint(th2, 10, &val_th2))
	{
		printk("invalid config th2 = %s, kstrtouint error\n", th2);
		return -EINVAL;
	}
    if(val_th1 > val_th2){
        printk("invalid config: TH1 > TH2\n");
        return count;
    }
...
int create_CMA_threshold_proc_entry(void)
{
	...
	printk("\033[35mCMA_threshold proc initialized\033[m\n");
					...
					unsigned long start, unsigned long end)
{
	...
	while (pfn < end || !list_empty(&cc->migratepages)) {	// !list_empty(&cc->migratepages) means migration failed, so retry it
		if (fatal_signal_pending(current)) {
			ret = -EINTR;
			break;
		}

		if (list_empty(&cc->migratepages)) {	// if list_empty(&cc->migratepages), find 32 new pages to be migrated
		       ...
		       unsigned migratetype)
{
	...
	// mark to MIGRAE_ISOLATE for [start, end], move isolated free_pages to free_area->free_list[ISOLATED], nr_free and nr_cma_free will not count these isolated free pages
