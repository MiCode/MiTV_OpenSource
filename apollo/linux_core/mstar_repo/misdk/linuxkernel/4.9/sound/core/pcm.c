					       ...
					       struct snd_info_buffer *buffer)
{
	...
	snd_iprintf(buffer, "-----\n");
	snd_iprintf(buffer, "open_time   : %ld.%09ld\n",
		runtime->open_tstamp.tv_sec, runtime->open_tstamp.tv_nsec);
	snd_iprintf(buffer, "xrun_cnt    : %ld\n", runtime->xrun_cnt);
	snd_iprintf(buffer, "pause_cnt   : %ld\n", runtime->pause_cnt);
	snd_iprintf(buffer, "suspend_cnt : %ld\n", runtime->suspend_cnt);
 unlock:
	mutex_unlock(&substream->pcm->open_mutex);
}
			     ...
			     struct snd_pcm_substream **rsubstream)
{
	...
	*rsubstream = NULL;
	...
	snd_pcm_gettime(runtime, &runtime->open_tstamp);
