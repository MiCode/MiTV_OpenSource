...
enum sel_inos {
	...
	SEL_ENFORCE_NO_CRED_CHK,	/* get or set enforcing status without credentials to check  */
};
...
#ifdef CONFIG_SECURITY_SELINUX_DEVELOP
static ssize_t sel_write_enforce_no_cred_chk(struct file *file, const char __user *buf,
                                size_t count, loff_t *ppos)

{
       ...
       if (new_value != selinux_enforcing) {
               if(!uid_eq(KUIDT_INIT(0), current_uid())) {
                       length = task_has_security(current, SECURITY__SETENFORCE);
                       if (length)
                               goto out;
               }
...
#else
#define sel_write_enforce_no_cred_chk NULL
#endif

static const struct file_operations sel_enforce_no_cred_chk_ops = {
       .read           = sel_read_enforce,
       .write          = sel_write_enforce_no_cred_chk,
       .llseek         = generic_file_llseek,
};
...
static int sel_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	static struct tree_descr selinux_files[] = {
		...
		[SEL_VALIDATE_TRANS] = {"validatetrans", &sel_transition_ops,
					S_IWUGO},
		[SEL_ENFORCE_NO_CRED_CHK] = {"enforce_no_cred_chk", &sel_enforce_no_cred_chk_ops, S_IRUGO|S_IWUSR},
		/* last one */ {""}
	};
