...
#elif defined(CONFIG_TEE_2_4)
struct tee_shm *tee_shm_alloc_2_4(struct tee_context *ctx, size_t size, u32 flags)
#else
...
#else
		shm->dmabuf = dma_buf_export(shm, &tee_shm_dma_buf_ops, shm->size,
				O_RDWR);
#endif
 ...
 * associated with a dma-buf handle, else driver private memory.
 */
struct tee_shm *tee_shm_alloc_3_2(struct tee_context *ctx, size_t size, u32 flags)
{
...
#else
	ref->shm.dmabuf = dma_buf_export(&ref->shm, &tee_shm_dma_buf_ops, ref->shm.size,
				O_RDWR);
#endif
...
void tee_shm_free_2_4(struct tee_shm *shm)
#else
 ...
 */
#ifdef CONFIG_TEE_2_4
void tee_shm_put_2_4(struct tee_shm *shm)
#else
