...
unsigned long bt_pw_event_en=1;
int bluetooth_pw_event_enable(unsigned int value){
   if(value==1){
       bt_pw_event_en = 1;
       return 1;
   }else{
...
EXPORT_SYMBOL(bluetooth_pw_event_enable);
			  ...
			  unsigned int type, unsigned int code, int *pval)
{
	...
	case EV_KEY:
		if (is_event_supported(code, dev->keybit, KEY_MAX)) {
            ...
            evdev_misc_debug("EV_KEY,code 0x%02x \n",code);
			if(input_event_en==0x1){
                if(bt_pw_event_en==0x0&&dev->id.bustype==BUS_BLUETOOTH&&code==KEY_POWER)
                    disposition = INPUT_IGNORE_EVENT;
				else if(do_block(code,0,1)){
					disposition = INPUT_IGNORE_EVENT;
				}
				else{
					disposition = INPUT_PASS_TO_HANDLERS;
	...
	case EV_MSC:
		if (is_event_supported(code, dev->mscbit, MSC_MAX))
		{
			...
			if(input_event_en==0x1){
                //if(bt_pw_event_en==0x0&&dev->id.bustype==BUS_BLUETOOTH)
                //    disposition = INPUT_IGNORE_EVENT;
				//else 
