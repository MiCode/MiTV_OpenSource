...
class SystemValues:
	...
	traceevents = [
		...
		'utpa_pm_callback_start'
		'utpa_pm_callback_end'
	]
	logmsg = ''
	testcommand = ''
	mempath = '/dev/mem'
...
def parseTraceLog(live=False):
	...
	for line in tf:
		...
		if(t.fevent):
			...
			# device callback start & UTPA callback start
			elif(t.type == 'device_pm_callback_start' or t.type == 'utpa_pm_callback_start'):
			...
			# device callback finish & UTPA callback end
			elif(t.type == 'device_pm_callback_end' or t.type == 'utpa_pm_callback_end'):
				if phase not in data.dmesg:
					continue
