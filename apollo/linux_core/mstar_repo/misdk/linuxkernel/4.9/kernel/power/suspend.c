...
static PM_STATE pm_state = E_PM_STATE_POWER_ON_DC;
#endif
...
static int suspend_prepare(suspend_state_t state)
{
	...
	printk(KERN_ERR "\033[0;32;31m%s %d Disable DVFS in STR\033[m\n",__func__,__LINE__);
	Mdrv_CpuFreq_All_UnLock((char *)__FUNCTION__);

	/* Reset cpufreq and voltage to default setting */
	printk(KERN_ERR "\033[36mFunction = %s, Line = %d,  setting cpufreq\033[m\n", __PRETTY_FUNCTION__, __LINE__);
...
#ifdef CONFIG_MSTAR_IPAPOOL
		printk("\033[0;32;31m%s %d error ,should never enable both CONFIG_MSTAR_CMAPOOL and CONFIG_MSTAR_IPAPOOL !\033[m\n",__func__,__LINE__);
		BUG();
#endif
#endif
 ...
 Finish:
	...
	printk(KERN_ERR "\033[0;32;31m%s %d Enable DVFS in STR (Error case)\033[m\n",__func__,__LINE__);
	Mdrv_CpuFreq_All_UnLock((char *)__FUNCTION__);
#endif
#endif
		...
		*wakeup = pm_wakeup_pending();
		if (!(suspend_test(TEST_CORE) || *wakeup)) {
			trace_suspend_resume(TPS("machine_suspend"),
				state, true);
#if defined(CONFIG_MSTAR_PM)
			MDrv_PM_Suspend(E_PM_STATE_SUSPEND_PREPARE);
#endif
#if defined(CONFIG_MSTAR_FRC_R2)
			MDrv_FRC_R2_Suspend(E_R2_STATE_SUSPEND_PREPARE);
#endif
...
static void suspend_finish(void)
{
	...
	Mdrv_CpuFreq_All_Lock((char *)__FUNCTION__);
	atomic_set(&disable_dvfs, 0);
	printk(KERN_ERR "\033[0;32;31m%s %d Enable DVFS in STR\033[m\n",__func__,__LINE__);
	Mdrv_CpuFreq_All_UnLock((char *)__FUNCTION__);
#endif
#endif
...
#if defined(CONFIG_MSTAR_PM)
static void pm_resume(void *data)
{
    PM_STATE *state = (PM_STATE *)data;
    MDrv_PM_Resume( *state );
}
#endif
...
#define STR_FLAG_SUSPEND_FINISH 0xFFFF8888
	if (TEEINFO_TYPTE == SECURITY_TEEINFO_OSTYPE_NUTTX) {
		...
		if(u64TEESTRBOOTFLAG != 0) {
			...
			printk(KERN_INFO "Normal STR flow\n");
	}
#endif
	...
	pm_restore_gfp_mask();
#if defined(CONFIG_MSTAR_PM)
	pm_state = E_PM_STATE_POWER_ON_DC;
	pm_task = kthread_create(pm_resume, &pm_state, "pm_task");

	if (IS_ERR(pm_task))
		pr_err("\033[0;32;31m%s %d PM Task Create Failed!!\033[m\n", __func__, __LINE__);

	wake_up_process(pm_task);
	//MDrv_PM_Resume(E_PM_STATE_POWER_ON_DC); mask it for str resume performance.
#endif
