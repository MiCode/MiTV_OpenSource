...
/* lalit: for keeping portability with original code and userspace tool */
#ifdef DEMANGLE_TOOL
#define dem_print printf
#else
#define dem_print PRINT_KD
#endif
...
#ifdef __GNUC__
/* lalit: dynamic array cause problem in kernel as it occupy the stack */
...
static struct demangle_component *d_make_builtin_type(struct d_info *, const struct
		demangle_builtin_type_info
		*);

static struct demangle_component *d_make_operator(struct d_info *, const struct
		demangle_operator_info
		*);

static struct demangle_component *d_make_extended_operator(struct d_info *, int, struct
		demangle_component
		*);

static struct demangle_component *d_make_ctor(struct d_info *,
		enum gnu_v3_ctor_kinds,
		struct demangle_component *);

static struct demangle_component *d_make_dtor(struct d_info *,
		enum gnu_v3_dtor_kinds,
		struct demangle_component *);

static struct demangle_component *d_make_template_param(struct d_info *, long);
...
static inline void d_growable_string_resize(struct d_growable_string *, size_t);
...
d_print_function_type(struct d_print_info *,
		const struct demangle_component *, struct d_print_mod *);
...
#ifdef KDBG_MEM_DBG
static void aop_dbg_check_stack_usage(void);
#endif
...
static void d_dump(struct demangle_component *dc, int indent)
{
	...
	case DEMANGLE_COMPONENT_TEMPLATE_PARAM:
		dem_print("template parameter %ld\n", dc->u.s_number.number);
		return;
	case DEMANGLE_COMPONENT_CTOR:
		dem_print("constructor %d\n", (int)dc->u.s_ctor.kind);
		d_dump(dc->u.s_ctor.name, indent + 2);
		return;
	case DEMANGLE_COMPONENT_DTOR:
		dem_print("destructor %d\n", (int)dc->u.s_dtor.kind);
		d_dump(dc->u.s_dtor.name, indent + 2);
		return;
	case DEMANGLE_COMPONENT_SUB_STD:
		dem_print("standard substitution %s\n", dc->u.s_string.string);
		return;
	case DEMANGLE_COMPONENT_BUILTIN_TYPE:
		dem_print("builtin type %s\n", dc->u.s_builtin.type->name);
		return;
	case DEMANGLE_COMPONENT_OPERATOR:
		dem_print("operator %s\n", dc->u.s_operator.op->name);
		return;
	case DEMANGLE_COMPONENT_EXTENDED_OPERATOR:
		dem_print("extended operator with %d args\n",
				dc->u.s_extended_operator.args);
		d_dump(dc->u.s_extended_operator.name, indent + 2);
		return;

	case DEMANGLE_COMPONENT_QUAL_NAME:
		dem_print("qualified name\n");
		break;
	case DEMANGLE_COMPONENT_LOCAL_NAME:
		dem_print("local name\n");
		break;
	case DEMANGLE_COMPONENT_TYPED_NAME:
		dem_print("typed name\n");
		break;
	...
	case DEMANGLE_COMPONENT_CONSTRUCTION_VTABLE:
		dem_print("construction vtable\n");
		break;
	case DEMANGLE_COMPONENT_TYPEINFO:
		dem_print("typeinfo\n");
		break;
	case DEMANGLE_COMPONENT_TYPEINFO_NAME:
		dem_print("typeinfo name\n");
		break;
	case DEMANGLE_COMPONENT_TYPEINFO_FN:
		dem_print("typeinfo function\n");
		break;
	case DEMANGLE_COMPONENT_THUNK:
		dem_print("thunk\n");
		break;
	case DEMANGLE_COMPONENT_VIRTUAL_THUNK:
		dem_print("virtual thunk\n");
		break;
	case DEMANGLE_COMPONENT_COVARIANT_THUNK:
		dem_print("covariant thunk\n");
		break;
	case DEMANGLE_COMPONENT_JAVA_CLASS:
		dem_print("java class\n");
		break;
	case DEMANGLE_COMPONENT_GUARD:
		dem_print("guard\n");
		break;
	case DEMANGLE_COMPONENT_REFTEMP:
		dem_print("reference temporary\n");
		break;
	case DEMANGLE_COMPONENT_HIDDEN_ALIAS:
		dem_print("hidden alias\n");
		break;
	...
	case DEMANGLE_COMPONENT_RESTRICT_THIS:
		dem_print("restrict this\n");
		break;
	case DEMANGLE_COMPONENT_VOLATILE_THIS:
		dem_print("volatile this\n");
		break;
	case DEMANGLE_COMPONENT_CONST_THIS:
		dem_print("const this\n");
		break;
	case DEMANGLE_COMPONENT_VENDOR_TYPE_QUAL:
		dem_print("vendor type qualifier\n");
		break;
	...
	case DEMANGLE_COMPONENT_VENDOR_TYPE:
		dem_print("vendor type\n");
		break;
	case DEMANGLE_COMPONENT_FUNCTION_TYPE:
		dem_print("function type\n");
		break;
	case DEMANGLE_COMPONENT_ARRAY_TYPE:
		dem_print("array type\n");
		break;
	case DEMANGLE_COMPONENT_PTRMEM_TYPE:
		dem_print("pointer to member type\n");
		break;
	case DEMANGLE_COMPONENT_FIXED_TYPE:
		dem_print("fixed-point type\n");
		break;
	case DEMANGLE_COMPONENT_ARGLIST:
		dem_print("argument list\n");
		break;
	case DEMANGLE_COMPONENT_TEMPLATE_ARGLIST:
		dem_print("template argument list\n");
		break;
	case DEMANGLE_COMPONENT_CAST:
		dem_print("cast\n");
		break;
	case DEMANGLE_COMPONENT_UNARY:
		dem_print("unary operator\n");
		break;
	case DEMANGLE_COMPONENT_BINARY:
		dem_print("binary operator\n");
		break;
	case DEMANGLE_COMPONENT_BINARY_ARGS:
		dem_print("binary operator arguments\n");
		break;
	case DEMANGLE_COMPONENT_TRINARY:
		dem_print("trinary operator\n");
		break;
	case DEMANGLE_COMPONENT_TRINARY_ARG1:
		dem_print("trinary operator arguments 1\n");
		break;
	...
	case DEMANGLE_COMPONENT_LITERAL_NEG:
		dem_print("negative literal\n");
		break;
	case DEMANGLE_COMPONENT_JAVA_RESOURCE:
		dem_print("java resource\n");
		break;
	case DEMANGLE_COMPONENT_COMPOUND_NAME:
		dem_print("compound name\n");
		break;
	case DEMANGLE_COMPONENT_CHARACTER:
		dem_print("character '%c'\n", dc->u.s_character.character);
		return;
	case DEMANGLE_COMPONENT_DECLTYPE:
		dem_print("decltype\n");
		break;
	case DEMANGLE_COMPONENT_PACK_EXPANSION:
		dem_print("pack expansion\n");
		break;
	}
...
static struct demangle_component *d_make_name(struct d_info *di, const char *s,
		int len)
{
	struct demangle_component *p;
...
static struct demangle_component *d_make_builtin_type(struct d_info *di, const struct
		demangle_builtin_type_info
		*type)
{
	struct demangle_component *p;
...
static struct demangle_component *d_make_operator(struct d_info *di, const struct
		demangle_operator_info
		*op)
{
	struct demangle_component *p;
...
/* <mangled-name> ::= _Z <encoding>
   ...
   TOP_LEVEL is non-zero when called at the top level.  */
	...
	CP_STATIC_IF_GLIBCPP_V3
struct demangle_component *cplus_demangle_mangled_name(struct d_info *di,
		int top_level)
{
...
static struct demangle_component *d_unqualified_name(struct d_info *di)
{
	...
	if (IS_DIGIT(peek))
		...
		if (ret != NULL && ret->type == DEMANGLE_COMPONENT_OPERATOR)
			di->expansion +=
				sizeof "operator" + ret->u.s_operator.op->len - 2;
		return ret;
...
static struct demangle_component *d_operator_name(struct d_info *di)
{
	...
	c2 = d_next_char(di);
	if (c1 == 'v' && IS_DIGIT(c2))
		return d_make_extended_operator(di, c2 - '0',
				d_source_name(di));
	else if (c1 == 'c' && c2 == 'v')
...
static struct demangle_component *d_java_resource(struct d_info *di)
{
	...
	while (len > 0) {
		...
		if (p == NULL)
			...
			p = d_make_comp(di, DEMANGLE_COMPONENT_COMPOUND_NAME, p,
					next);
			if (p == NULL)
				return NULL;
		}
...
static struct demangle_component *d_special_name(struct d_info *di)
{
	...
	if (d_check_char(di, 'T')) {
		...
		case 'R':
			return d_make_comp(di, DEMANGLE_COMPONENT_REFTEMP,
					d_name(di), NULL);
...
names.  */
	...
	case 'z':
		ret = d_make_builtin_type(di,
				&cplus_demangle_builtin_types[peek -
				'a']);
	...
	case 'S':
		   ...
		   <class-enum-type>.  */
		{
			...
			if (IS_DIGIT(peek_next)
					|| peek_next == '_' || IS_UPPER(peek_next)) {
				...
				if (ret != NULL
						&& ret->type == DEMANGLE_COMPONENT_SUB_STD)
					can_subst = 0;
			}
		}
		break;
	...
	case 'D':
		...
		case 'F':
			...
			ret->u.s_fixed.accum = IS_DIGIT(d_peek_char(di));
...
static struct demangle_component **d_cv_qualifiers(struct d_info *di, struct demangle_component
		**pret, int member_fn)
{
	char peek;
		...
		int has_return_type)
{
	...
	while (1) {
		...
		if (has_return_type) {
			...
			*ptl =
				d_make_comp(di, DEMANGLE_COMPONENT_ARGLIST, type,
						NULL);
			if (*ptl == NULL)
				return NULL;
			ptl = &d_right(*ptl);
...
static struct demangle_component *d_template_args(struct d_info *di)
{
	...
	while (1) {
		...
		*pal =
			d_make_comp(di, DEMANGLE_COMPONENT_TEMPLATE_ARGLIST, a,
					NULL);
		if (*pal == NULL)
			return NULL;
		pal = &d_right(*pal);
...
static struct demangle_component *d_exprlist(struct d_info *di)
{
...
static struct demangle_component *d_expression(struct d_info *di)
{
	...
	if (peek == 'L')
		...
		if (name == NULL)
			return NULL;
		if (d_peek_char(di) == 'I')
			return d_make_comp(di, DEMANGLE_COMPONENT_TEMPLATE,
					name, d_template_args(di));
		else
			return name;
	} else {
		...
		case 1:
			{
				...
				return d_make_comp(di, DEMANGLE_COMPONENT_UNARY,
						op, operand);
			}
		case 2:
			{
...
static struct demangle_component *d_expr_primary(struct d_info *di)
{
	...
	if (d_peek_char(di) == '_'
		...
		ret =
			d_make_comp(di, t, type, d_make_name(di, s, d_str(di) - s));
	}
	if (!d_check_char(di, 'E'))
		return NULL;
	return ret;
...
d_growable_string_resize(struct d_growable_string *dgs, size_t need)
{
	...
	newbuf = (char *)aop_dem_realloc(dgs->buf, dgs->alc, newalc);
	if (newbuf == NULL) {
		KDBG_MEM_DBG_KFREE(dgs->buf);
		dgs->buf = NULL;
		dgs->len = 0;
		dgs->alc = 0;
		...
		demangle_callbackref callback, void *opaque)
{
	...
	/* SISC: added for removing compiler strict warning */
	dpi->buf[0] = '\0';
	dpi->pack_index = 0;
}
...
/* Turn components into a human readable string.  OPTIONS is the
   ...
   failure.  */
	...
	CP_STATIC_IF_GLIBCPP_V3
char *cplus_demangle_print(int options, const struct demangle_component *dc,
...
static struct demangle_component *d_index_template_argument(struct
		demangle_component
		*args, int i)
{
	struct demangle_component *a;

	for (a = args; a != NULL; a = d_right(a)) {
...
static struct demangle_component *d_lookup_template_argument(struct d_print_info
		*dpi, const struct
		demangle_component
		*dc)
{
	...
	return d_index_template_argument
		(d_right(dpi->templates->template_decl), dc->u.s_number.number);
}

/* Returns a template argument pack used in DC (any will do), or NULL.  */

static struct demangle_component *d_find_pack(struct d_print_info *dpi, const struct demangle_component
		*dc)
{
	struct demangle_component *a;
	if (dc == NULL)
...
d_print_comp(struct d_print_info *dpi, const struct demangle_component *dc)
{
	...
	case DEMANGLE_COMPONENT_NAME:
		if ((dpi->options & DMGL_JAVA) == 0)
			...
			d_print_java_identifier(dpi, dc->u.s_name.s,
					dc->u.s_name.len);
		return;
	...
	case DEMANGLE_COMPONENT_TYPED_NAME:
		{
			...
			while (typed_name != NULL) {
				/* SISC: condition changed (functionally same) for removing compiler warning */
	...
	case DEMANGLE_COMPONENT_TEMPLATE_PARAM:
		{
			...
			struct demangle_component *a =
				d_lookup_template_argument(dpi, dc);

			if (a && a->type == DEMANGLE_COMPONENT_TEMPLATE_ARGLIST)
				a = d_index_template_argument(a,
						dpi->pack_index);
	...
	case DEMANGLE_COMPONENT_CONST:
		{
			...
			for (pdpm = dpi->modifiers; pdpm != NULL;
					pdpm = pdpm->next) {
				if (!pdpm->printed) {
					if (pdpm->mod->type !=
							DEMANGLE_COMPONENT_RESTRICT
	...
	case DEMANGLE_COMPONENT_FUNCTION_PARAM:
		{
			...
			snprintf(buf, 25, "%ld", dc->u.s_number.number);
			buf[24] = 0;
...
/* Print a list of modifiers.  SUFFIX is 1 if we are printing
   qualifiers on this after printing a function.  */
	...
	static void
d_print_mod_list(struct d_print_info *dpi, struct d_print_mod *mods, int suffix)
{
	struct d_print_template *hold_dpt;
...
/* Print an operator in an expression.  */
	...
	static void
d_print_expr_op(struct d_print_info *dpi, const struct demangle_component *dc)
{
		...
		demangle_callbackref callback, void *opaque)
{
	...
	struct d_info *pdi = NULL;
	struct demangle_component *dc = NULL;
	int status;
	pdi =
		KDBG_MEM_DBG_KMALLOC(KDBG_MEM_DEMANGLER_MODULE,
				sizeof(struct d_info), GFP_KERNEL);
	if (!pdi) {
		sym_errk("memory failure for di\n");
		return 0;
	}
	if (mangled[0] == '_' && mangled[1] == 'Z')
		type = DCT_MANGLED;
	else if (strncmp(mangled, "_GLOBAL_", 8) == 0
			&& (mangled[8] == '.' || mangled[8] == '_'
				|| mangled[8] == '$')
		...
		if ((options & DMGL_TYPES) == 0) {
			KDBG_MEM_DBG_KFREE(pdi);
			return 0;
		}
		type = DCT_TYPE;
	}

	cplus_demangle_init_info(mangled, options, strlen(mangled), pdi);

	{
		/* lalit: loal array cause too much stack and may crash the kernel
		   for long complex C++ template symbol.  So changing alloca to vmalloc.
		   We do not use CP_DYNAMIC_ARRAYS option, it is too dangerous to use in kernel  code.
		   */
#ifdef CP_DYNAMIC_ARRAYS
		/*__extension__ struct demangle_component comps[di.num_comps];*/
		/*__extension__ struct demangle_component *subs[di.num_subs];*/
		__extension__ struct demangle_component comps[pdi->num_comps];
		__extension__ struct demangle_component *subs[pdi->num_subs];
		...
		pdi->comps = KDBG_MEM_DBG_KMALLOC(KDBG_MEM_DEMANGLER_MODULE,
				pdi->num_comps *
				sizeof(*pdi->comps),
				GFP_KERNEL);
		pdi->subs =
			KDBG_MEM_DBG_KMALLOC(KDBG_MEM_DEMANGLER_MODULE,
					pdi->num_subs * sizeof(*pdi->subs),
					GFP_KERNEL);
#endif

		switch (type) {
		case DCT_TYPE:
			dc = cplus_demangle_type(pdi);
			break;
		case DCT_MANGLED:
			dc = cplus_demangle_mangled_name(pdi, 1);
			break;
		...
		case DCT_GLOBAL_DTORS:
			...
			dc = d_make_comp(pdi,
					...
					d_make_name(pdi, d_str(pdi),
						strlen(d_str(pdi))), NULL);
			d_advance(pdi, strlen(d_str(pdi)));
			break;
		}
		...
		/* If DMGL_PARAMS is set, then if we didn't consume the entire
		   ...
		   parameters.  */
		if (((options & DMGL_PARAMS) != 0) && d_peek_char(pdi) != '\0')
			dc = NULL;
		...
		status = (dc != NULL)
			? cplus_demangle_print_callback(options, dc, callback,
					opaque)
			: 0;
	}
	/* lalit: to avoid alloca(), so we use dyamic memory; */
#ifndef CP_DYNAMIC_ARRAYS
	KDBG_MEM_DBG_KFREE(pdi->comps);
	KDBG_MEM_DBG_KFREE(pdi->subs);
#endif
	KDBG_MEM_DBG_KFREE(pdi);
		...
		size_t *length, int *status)
{
	...
	if (output_buffer == NULL) {
		...
		if (strlen(demangled) < *length) {
			...
			KDBG_MEM_DBG_KFREE(demangled);
			demangled = output_buffer;
		} else {
			KDBG_MEM_DBG_KFREE(output_buffer);
			*length = alc;
		}
	}
...
extern int __gcclibcxx_demangle_callback(const char *, void (*)
		(const char *, size_t, void *),
		void *);
...
char *java_demangle_v3(const char *mangled)
{
	...
	return d_demangle(mangled, DMGL_JAVA | DMGL_PARAMS | DMGL_RET_POSTFIX,
			&alc);
}
		...
		enum gnu_v3_dtor_kinds *dtor_kind)
{
	...
	*dtor_kind = (enum gnu_v3_dtor_kinds)0;
...

	{
		...
		/* di.comps = alloca (di.num_comps * sizeof (*di.comps)); */
		/* di.subs = alloca (di.num_subs * sizeof (*di.subs)); */
		di.comps = KDBG_MEM_DBG_KMALLOC(KDBG_MEM_DEMANGLER_MODULE,
				di.num_comps *
				sizeof(*di.comps), GFP_KERNEL);
		di.subs =
			KDBG_MEM_DBG_KMALLOC(KDBG_MEM_DEMANGLER_MODULE,
					di.num_subs * sizeof(*di.subs),
					GFP_KERNEL);
#endif
	...
	/* lalit: to avoid alloca(), so we use dyamic memory; */
#ifndef CP_DYNAMIC_ARRAYS
	KDBG_MEM_DBG_KFREE(di.comps);
	KDBG_MEM_DBG_KFREE(di.subs);
#endif
...
int aop_dem_dummy_main(int argc, char *argv[])
{
	int i;
	int opt_char;
...
#ifdef IN_GLIBCPP_V3
				s = __cxa_demangle(dyn_string_buf(mangled),
						NULL, NULL, NULL);
#else
				s = cplus_demangle_v3(dyn_string_buf(mangled),
						options);
#endif

				if (s != NULL) {
					fputs(s, stdout);
					KDBG_MEM_DBG_KFREE(s);
				} else {
...
{
	int free = 0;
	unsigned long stack_start = (unsigned long)end_of_stack(current);

	free = (unsigned long)&free - stack_start;

	if (free < (3 * 1024)) {
		unsigned long stack_end =
			(unsigned long)task_stack_page(current) + THREAD_SIZE;
		PRINT_KD
			("\n%s: WARNING: stack free: %u  [start= %lu, end= %lu, cur= %lu]\n",
			 __FUNCTION__, free, stack_end, stack_start,
			 (unsigned long)&free);
	}
}
#endif
