...
extern int eas_boost_value;
#endif

// Unlike schedtune_task_boost(), we don't use "eas_boost_value" to be cpu boost value.
// In Adaptvie EAS algorithm, eas_boost_value will be very high when loading is lower.
// As a result, schedutil will get high cpu utility to adjust frequency to maximal value
// even if the system is at standby mode.
...
int schedtune_task_boost(struct task_struct *p)
{
	...
	task_boost = st->boost;
#if defined(CONFIG_DEFAULT_USE_ENERGY_AWARE) && defined(CONFIG_MP_EAS_BOOST_PERFORMANCE)
    // Assume user will not set boost to 0 explicitly since 0 is boost default value.
    // Alternatively, you can use an extra flag to select task boost value or system-wide boost value.
    if (task_boost == 0)
        task_boost = eas_boost_value;
#endif
	rcu_read_unlock();
	return task_boost;
}
...
int schedtune_set_task_boost(struct task_struct *p, s64 boost)
{
	struct schedtune *st;
