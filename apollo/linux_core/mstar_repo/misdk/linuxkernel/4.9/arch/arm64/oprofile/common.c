...
static void arm_backtrace(struct pt_regs * const regs, unsigned int depth)
{
	...
	if (!user_mode(regs)) {
		...
		/*struct stackframe in arm64 has no member which is named lr*/
		//frame.lr = regs->ARM_lr;
		frame.pc = regs->ARM_pc;
		walk_stackframe(current, &frame, report_trace, &depth);	// 4.4 need to pass a tsk parameter, here we pass current_tsk
		return;
	}
