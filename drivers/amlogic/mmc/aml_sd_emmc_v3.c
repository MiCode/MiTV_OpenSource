		...
		unsigned long *fixed_adj_map, u32 div)
{
	...
	DECLARE_BITMAP(cur_map, div);
	DECLARE_BITMAP(prev_map, div);
	DECLARE_BITMAP(tmp, div);
	DECLARE_BITMAP(dst, div);

	bitmap_zero(dst, div);
	bitmap_zero(tmp, div);
	bitmap_zero(cur_map, div);
	bitmap_zero(prev_map, div);
