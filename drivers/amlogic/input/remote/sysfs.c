...
static bool remote_enable = 1; /* default enable */
...
static void enable_ir_irq(struct device *dev, int state)
{
	struct remote_chip *chip = dev_get_drvdata(dev);
				...
				const char *buf, size_t count)
{
	...
	enable_ir_irq(dev, state);

	return strnlen(buf, count);
}

DEVICE_ATTR_RW(remote_enable);
