...
/* Follow EDID Spec Release Rev.2 @ 2006 */
#define EDID_ADDR_HOR_SIZE 0x15
#define EDID_ADDR_VER_SIZE 0x16
#define EDID_ADDR_CHECKSUM 0x7F
...
/* use factor of 0.138176 and multiply by panel size */
static unsigned char panel_size_info[10][2] = {
	{71, 40},  /* 32 */
	{89, 50},  /* 40 */
	{95, 54},  /* 43 */
	{111, 62}, /* 50 */
...
static int __init get_mi_panel_size(char *str)
{
	int ret = 0;

	if (str != NULL)
		ret = kstrtouint(str, 10, &panel_size);

	return ret;
}
__setup("androidboot.mi.panel_size=", get_mi_panel_size);
...
unsigned int hdmi_rx_top_edid_update(void)
{
	...
	/* update panel size info to real value */
	rx_pr("update edid for panel size : %d\n", panel_size);
	switch (panel_size) {
	case 32:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_32][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_32][1];
		break;
	case 40:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_40][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_40][1];
		break;
	case 43:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_43][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_43][1];
		break;
	case 50:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_50][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_50][1];
		break;
	case 55:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_55][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_55][1];
		break;
	case 58:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_58][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_58][1];
		break;
	case 65:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_65][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_65][1];
		break;
	case 75:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_75][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_75][1];
		break;
	case 49:
		pedid_data[EDID_ADDR_HOR_SIZE] = panel_size_info[SIZE_49][0];
		pedid_data[EDID_ADDR_VER_SIZE] = panel_size_info[SIZE_49][1];
		break;
	default:
		pedid_data[EDID_ADDR_HOR_SIZE] = 0;
		pedid_data[EDID_ADDR_VER_SIZE] = 0;
		break;
	}
	for (i = 0; i < EDID_ADDR_CHECKSUM; i++)
		edid_checksum += pedid_data[i];
	/* checksum=256-(byte0+byte1+...+byt126)%256 */
	pedid_data[EDID_ADDR_CHECKSUM] = 256 - edid_checksum%256;
