...
#if 1
	if (!tunnel_drop_done && video_vf_peek()) {

		struct vframe_s *vf = NULL;
		u32 first_apts = timestamp_firstapts_get();
		//pr_info("=====first apts:%x\n",
		//                    first_apts);
		while (first_apts != 0) {
			vf = video_vf_peek();
			if (vf) {
				//pr_info("===== %x, first apts:%x\n",
				//       vf->pts, first_apts);
				if (vf->pts < first_apts) {
					/*
					pr_info("vsync drop omx_index %x,first apts:%x\n",
							vf->pts, first_apts);
					*/
					timestamp_videodrop_pts_set(vf->pts);
					timestamp_vpts_set(vf->pts);
					vf = video_vf_get();
					video_vf_put(vf);
				} else {
					//pr_info("====end drop\n");
					//tunnel_drop_done = true;
					break;
				}
			} else
				break;
...
static void set_omx_pts(u32 *p)
{
	...
	if (debug_flag & DEBUG_FLAG_PTS_TRACE)
		pr_info("[set_omx_pts]tmp_pts:%d, set_from_hwc:%d,frame_num=%d, not_reset=%d, pcr-omx-diff=%d\n",
			tmp_pts, set_from_hwc, frame_num, not_reset,
			timestamp_pcrscr_get() - omx_pts);
