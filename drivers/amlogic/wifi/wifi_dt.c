...
static int wifi_setup_dt(void)
{
	...
	if (wifi_info.interrupt_pin) {
		...
		ret = gpiod_set_pull(wifi_info.interrupt_desc, GPIOD_PULL_UP);
		if (ret)
			WIFI_INFO("interrupt_pin pull up failed(%d)\n", ret);
...
static void wifi_dev_shutdown(struct platform_device *pdev)
{
	WIFI_INFO("wifi_dev_shutdown\n");
	set_power(0);
}
